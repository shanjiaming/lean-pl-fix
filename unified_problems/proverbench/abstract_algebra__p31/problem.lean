    ...
    have h₁₁ : P ∣ Q → ∃ i : ℕ, n = 6 * i - 1 := by
      ...
      have h₄ : n = 0 := by
        ...
        have h₅ : n ≥ 1 := by
          by_contra h₅
          have h₆ : n = 0 := by omega
          simp_all [h₆, pow_zero, pow_one, mul_zero, Nat.mul_succ]
          <;>
          (try norm_num) <;>
          ...
          (try {
            use 0
            <;> ring_nf at * <;> norm_num at * <;> simp_all [dvd_iff_exists_eq_mul_left]
            <;>
            aesop
          })
        have h₆ := h₃
        ...
        <;>
        (try {
          cases n <;> simp_all [Nat.mul_succ, pow_add, pow_one, pow_mul, Nat.mul_zero, Nat.add_zero]
    ...
    have h₁₂ : (∃ i : ℕ, n = 6 * i - 1) → P ∣ Q := by
      ...
      have h₅ : i = 0 := by
        ...
        have h₆ : i ≥ 1 := by
          by_contra h₆
          have h₇ : i = 0 := by omega
          simp_all [h₇, mul_zero, Nat.mul_succ]
          <;> ring_nf at * <;> norm_num at * <;> omega
        have h₇ : n ≥ 5 := by
          omega
        have h₈ : (X ^ n + X - 1 : Polynomial R) ∣ (X ^ 2 - X + 1 : Polynomial R) := by
          exfalso
          have h₉ := h₇
          have h₁₀ : n ≥ 5 := by omega
          have h₁₁ : (X ^ n + X - 1 : Polynomial R) ∣ (X ^ 2 - X + 1 : Polynomial R) → False := by
            intro h₁₁
            have h₁₂ : (X ^ n + X - 1 : Polynomial R) ∣ (X ^ 2 - X + 1 : Polynomial R) := by simpa using h₁₁
            rw [dvd_iff_exists_eq_mul_left] at h₁₂
            rcases h₁₂ with ⟨p, hp⟩
