theorem f_global_minimum :
  ...
  have h_main : ∀ (p : ℝ × ℝ), f p.1 p.2 ≥ f 0 0 := by
    ...
    have h₁ : f p.1 p.2 = (p.1 ^ 2 + p.2 ^ 2) * Real.exp (-(p.1 + p.2)) := by
      simp [f, Real.exp_neg, add_comm]
      <;> ring_nf
      <;> field_simp [Real.exp_add, Real.exp_neg, add_comm]
      <;> ring_nf
    have h₂ : f (0 : ℝ) 0 = 0 := by
  ...
  have h_final : IsMinOn (fun p => f p.1 p.2) Set.univ (0, 0) := by
    refine' fun p _ => h_main p
