theorem putnam_1994_a4
  ...
  have h_det_A : A.det = 1 ∨ A.det = -1 := by
    have h₁ : IsUnit A.det := by
      -- Prove that the determinant of A is a unit in the integers (i.e., ±1)
      have h₂ : Invertible A := ABinv.1.some
      have h₃ : IsUnit A.det := by
        -- Use the fact that A is invertible to show that its determinant is a unit
        exact?
      exact h₃
    -- Since the determinant of A is a unit in the integers, it must be either 1 or -1
  ...
  have h_d : A 0 0 * B 1 1 + A 1 1 * B 0 0 - A 0 1 * B 1 0 - A 1 0 * B 0 1 = 0 := by
    ...
    have h₆ : B.det = B 0 0 * B 1 1 - B 0 1 * B 1 0 := by
      simp [Matrix.det_fin_two]
      <;> ring
    -- Use the given conditions to derive the desired result
