theorem real_numbers_equation_solution :
  ...
  have h₂ : 3 * (b^2 + c^2 + d^2) - 2 * (b * c + b * d + c * d) ≥ 0 := by
    nlinarith [sq_nonneg (b - c), sq_nonneg (b - d), sq_nonneg (c - d),
      sq_nonneg (b + c + d), sq_nonneg (b + c - d), sq_nonneg (b + d - c),
      sq_nonneg (c + d - b)]
  
  have h₃ : a = (b + c + d) / 2 := by
    have h₄ : (a - (b + c + d) / 2) ^ 2 ≥ 0 := by
      nlinarith
    have h₅ : (3 * (b ^ 2 + c ^ 2 + d ^ 2) - 2 * (b * c + b * d + c * d)) / 4 ≥ 0 := by
      nlinarith
    have h₆ : (a - (b + c + d) / 2) ^ 2 = 0 := by
      nlinarith
    have h₇ : a - (b + c + d) / 2 = 0 := by
      nlinarith
    linarith
  ...
  have h₇ : c = 0 := by
    ...
    have h₇₂ : b = 0 := h₆
    rw [h₇₂] at h₇₁
    have h₇₃ : (0 - c) ^ 2 + (0 - d) ^ 2 + (c - d) ^ 2 + (0 ^ 2 + c ^ 2 + d ^ 2) = 0 := by
      ring_nf at h₇₁ ⊢
      <;> linarith
