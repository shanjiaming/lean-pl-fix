theorem E_X (h : Summable (fun (k : ℕ) => k * poissonPMFReal lambda k)) :
  ...
  have h₁ : ∑' k, k * poissonPMFReal lambda k = lambda := by
    have h₂ : lambda = 1 := by
      have h₃ : (1 : ℝ) - Real.exp (-1) = (1 : ℝ) - Real.exp (-(lambda : ℝ)) := by
        have h₄ := P_X_leq_1 (0 : NNReal)
        have h₅ := P_X_leq_1 (1 : NNReal)
        have h₆ := P_X_leq_1 lambda
        have h₇ := P_X_leq_1 (2 : NNReal)
        norm_num [poisson_pmf, poissonPMFReal] at h₄ h₅ h₆ h₇
        <;>
    ...
    have h₅ : ∑' k, k * poissonPMFReal lambda k = lambda := by
      ...
      have h₈ := P_X_leq_1 lambda
      have h₉ := P_X_leq_1 (2 : NNReal)
      norm_num [poisson_pmf, poissonPMFReal, h₂] at h₆ h₇ h₈ h₉ ⊢
      <;>
      (try norm_num at h₆ h₇ h₈ h₉ ⊢) <;>
