theorem mulVec_smul (A : Matrix n n R) (k : R) (x : n → R) : A.mulVec (k • x) = k • (A.mulVec x):=
  by
  have h_main : ∀ (i : n), (A.mulVec (k • x)) i = (k • (A.mulVec x)) i:=
    by
    intro i
    calc
    ...
    hole
  have h_final : A.mulVec (k • x) = k • (A.mulVec x):=
    by
    apply funext
    intro i
    apply h_main i <;> done
