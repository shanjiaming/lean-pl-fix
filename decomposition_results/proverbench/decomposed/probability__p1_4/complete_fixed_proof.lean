theorem cov_X_X2 (h0 : lambda = 1) (h1 : Summable fun k => k * k ^ 2 * poissonPMFReal lambda k)
  (h2 : Summable fun k => k * poissonPMFReal lambda k) (h3 : Summable fun k => k ^ 2 * poissonPMFReal lambda k) :
  (∑' k, k * k ^ 2 * poissonPMFReal lambda k) -
      (∑' k, k * poissonPMFReal lambda k) * (∑' k, k ^ 2 * poissonPMFReal lambda k) =
    3:=
  by
  have h4 : (∑' k, k * poissonPMFReal lambda k) = 1:=
    by
    have h4₁ : ∑' k, k * poissonPMFReal lambda k = lambda:=
      by
      --  apply E_X
      --  exact h2
      hole
    --  --  rw [h4₁] <;> simp [h0] <;> norm_num <;> simp_all [poissonPMFReal] <;> norm_num <;> simp_all [poissonPMFReal] <;>
      ...
      norm_num
    hole
  have h5 : (∑' k, k ^ 2 * poissonPMFReal lambda k) = 2:=
    by
    have h5₁ : lambda = 1 := h0
    --  subst h5₁
    have h5₂ : (∑' k : ℕ, (k : ℝ) ^ 2 * poissonPMFReal (1 : NNReal) k) = 2:=
      by
      have h5₃ : (∑' k : ℕ, (k : ℝ) ^ 2 * poissonPMFReal (1 : NNReal) k) = 2:=
        by
        have h5₄ : (∑' k : ℕ, (k : ℝ) ^ 2 * poissonPMFReal (1 : NNReal) k) = 2:=
    ...
    --  simpa using h5₂
    hole
  have h6 : (∑' k, k * k ^ 2 * poissonPMFReal lambda k) = 5:=
    by
    have h6₁ : lambda = 1 := h0
    --  subst h6₁
    have h6₂ : (∑' k : ℕ, (k : ℝ) * k ^ 2 * poissonPMFReal (1 : NNReal) k) = 5:=
      by
      have h6₃ : (∑' k : ℕ, (k : ℝ) * k ^ 2 * poissonPMFReal (1 : NNReal) k) = 5:=
        by
        have h6₄ : (∑' k : ℕ, (k : ℝ) * k ^ 2 * poissonPMFReal (1 : NNReal) k) = 5:=
