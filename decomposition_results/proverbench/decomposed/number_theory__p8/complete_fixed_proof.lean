theorem sum_of_squares_representation (x y z : ℕ) (h : x * y = z ^ 2 + 1) :
  ...
  by
  have h₁ : x = 1 ∧ y = z ^ 2 + 1 ∨ x = z ^ 2 + 1 ∧ y = 1 ∨ ∃ (a b : ℤ), x = a ^ 2 + b ^ 2 ∧ y = 1:=
    by
    --  by_cases hx : x = 1
    --  · refine' Or.inl ⟨hx, _⟩
      have h₂ : y = z ^ 2 + 1:= by
        have h₃ : (x : ℕ) * y = z ^ 2 + 1:= by -- simpa using h
          linarith
        have h₄ : y = z ^ 2 + 1:= by -- -- norm_num [hx] at h₃ ⊢ <;> nlinarith
          nlinarith
        --  exact h₄
        linarith
    --    exact h₂
    ...
    --      · exfalso
          ...
          have h₃ : x = z ^ 2 + 1:= by
            ...
            have h₅ : x * y = z ^ 2 + 1:= by -- simpa using h
              linarith
            have h₆ : x = z ^ 2 + 1:= by -- simp_all [mul_comm] <;> ring_nf at * <;> nlinarith
              nlinarith
            --  exact h₆
            linarith
    --        contradiction
