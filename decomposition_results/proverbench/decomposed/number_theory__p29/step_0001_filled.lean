theorem rational_solution_equivalence (d : ℤ) (x y : ℚ) (h : x ≠ 1) : x ^ 2 - ↑d * y ^ 2 = 1 ↔ ∃ t, x = (↑d * t ^ 2 + 1) / (↑d * t ^ 2 - 1) ∧ y = 2 * t / (↑d * t ^ 2 - 1) :=
  ...
  have h_main :
    ...
    have h₃ :
      ...
      intro h₄ h₅
      have h₆ : x ^ 2 = 1 := by
        simp [h₄] at h₅ ⊢ <;> norm_num at h₅ ⊢ <;> (try simp_all) <;> (try nlinarith) <;> (try linarith) <;>
          (try nlinarith)
      have h₇ : x = 1 ∨ x = -1 := by
        have h₈ : x ^ 2 = 1 := h₆
        have h₉ : x = 1 ∨ x = -1 := by
      ...
      | inr h₇ =>
        ...
        constructor
        ·
          simp [h₄, h₇] <;> field_simp [h₂, sub_eq_zero, add_eq_zero_iff_eq_neg] at h₁ ⊢ <;> ring_nf at h₁ ⊢ <;>
            nlinarith
        ·
          simp [h₄, h₇] <;> field_simp [h₂, sub_eq_zero, add_eq_zero_iff_eq_neg] at h₁ ⊢ <;> ring_nf at h₁ ⊢ <;>
            nlinarith
    ...
    ·
      have h₅ : x - 1 ≠ 0 := by
        ...
        linarith
      use y / (x - 1)
      have h₆ : x = (↑d * (y / (x - 1)) ^ 2 + 1) / (↑d * (y / (x - 1)) ^ 2 - 1) :=
        by
        ...
        have h₁₁ : ↑d * (y / (x - 1)) ^ 2 - 1 ≠ 0 := by
          by_contra h₁₁
          have h₁₂ : ↑d * (y / (x - 1)) ^ 2 = 1 := by linarith
          have h₁₃ : x ^ 2 - ↑d * y ^ 2 = 1 := h₁
          have h₁₄ : x = 1 := by
        ...
        have h₁₂ : x = (↑d * (y / (x - 1)) ^ 2 + 1) / (↑d * (y / (x - 1)) ^ 2 - 1) := by
          field_simp [h₁₁, h₅, sub_eq_zero, add_eq_zero_iff_eq_neg] at h₁ ⊢ <;> ring_nf at h₁ ⊢ <;>
            nlinarith [sq_pos_of_ne_zero (sub_ne_zero.mpr h₂), sq_pos_of_ne_zero h₉]
        exact h₁₂
      have h₇ : y = 2 * (y / (x - 1)) / (↑d * (y / (x - 1)) ^ 2 - 1) :=
        by
        have h₈ : x ^ 2 - ↑d * y ^ 2 = 1 := h₁
        have h₉ : x ≠ 1 := h₂
        have h₁₀ : y ≠ 0 := h₄
      ...
      exact ⟨h₆, h₇⟩
  have h_main' :
    (∃ t : ℚ, x = (d * t ^ 2 + 1) / (d * t ^ 2 - 1) ∧ y = 2 * t / (d * t ^ 2 - 1)) → (x ^ 2 - d * y ^ 2 = 1) := by sorry
  have h_main : (x ^ 2 - d * y ^ 2 = 1) → ∃ t : ℚ, x = (d * t ^ 2 + 1) / (d * t ^ 2 - 1) ∧ y = 2 * t / (d * t ^ 2 - 1) := by sorry
have rational_solution_equivalence (h : x ≠ 1) :
  (x ^ 2 - d * y ^ 2 = 1) ↔ ∃ t : ℚ, x = (d * t ^ 2 + 1) / (d * t ^ 2 - 1) ∧ y = 2 * t / (d * t ^ 2 - 1) :=
  by
  have h_main :
    (x ^ 2 - d * y ^ 2 = 1) → ∃ t : ℚ, x = (d * t ^ 2 + 1) / (d * t ^ 2 - 1) ∧ y = 2 * t / (d * t ^ 2 - 1) :=
