theorem product_of_six_consecutive_integers_not_fifth_power :
  ...
  have h₂ : a * b * c * d * e * f = a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) := by
    have h₃ : a + 1 = b := h_consecutive.1
    have h₄ : b + 1 = c := h_consecutive.2.1
    have h₅ : c + 1 = d := h_consecutive.2.2.1
    have h₆ : d + 1 = e := h_consecutive.2.2.2.1
  ...
  have h₃ : a = 0 := by
    ...
    have h₄ : a ≥ 1 := by
      omega
    have h₅ : a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) = n ^ 5 := by
      linarith
    -- Use the fact that the product of six consecutive integers cannot be a perfect fifth power
    have h₆ : False := by
      -- Use the fact that the product of six consecutive integers cannot be a perfect fifth power
      have h₇ : n ≥ 1 := by
        by_contra hn
        have hn' : n = 0 := by
          omega
        rw [hn'] at h₅
        have h₈ : a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) = 0 := by
          simp at h₅ ⊢
          <;> nlinarith
        have h₉ : a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) > 0 := by
          have h₁₀ : a ≥ 1 := by omega
      ...
      have h₉ : False := by
        -- Use the fact that the product of six consecutive integers cannot be a perfect fifth power
        have h₁₀ : 5 ∣ a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) := by
          -- Use the fact that the product of six consecutive integers is divisible by 5
          apply Nat.dvd_of_mod_eq_zero
          have h₁₁ : a % 5 = 0 ∨ a % 5 = 1 ∨ a % 5 = 2 ∨ a % 5 = 3 ∨ a % 5 = 4 := by omega
          rcases h₁₁ with (h₁₁ | h₁₁ | h₁₁ | h₁₁ | h₁₁) <;>
            simp [h₁₁, Nat.mul_mod, Nat.add_mod, Nat.mod_mod, Nat.pow_succ, Nat.mod_eq_of_lt,
              Nat.succ_pos, Nat.zero_lt_succ, Nat.mod_self, Nat.mod_one, Nat.mod_zero]
          <;> norm_num <;>
            (try omega) <;>
        ...
        have h₁₃ : n ^ 5 ≠ a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) := by
          -- Use the fact that the product of six consecutive integers cannot be a perfect fifth power
          intro h
          have h₁₄ : a ≥ 1 := by omega
                ...
                have h₂₃ : a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) > n ^ 5 := by
                  have h₂₄ : n < a + 3 := by
                    ...
                    have h₂₆ : n ^ 5 ≥ (a + 3) ^ 5 := by
                      calc
                        n ^ 5 ≥ (a + 3) ^ 5 := by
                          exact Nat.pow_le_pow_of_le_left h₂₅ 5
                        _ = (a + 3) ^ 5 := by ring
                    have h₂₇ : (a + 3) ^ 5 > a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) := by
                      have h₂₈ : a ≥ 6 := by omega
                      have h₂₉ : (a + 3) ^ 5 > a * (a + 1) * (a + 2) * (a + 3) * (a + 4) * (a + 5) := by
                        nlinarith [sq_nonneg (a ^ 2), sq_nonneg ((a + 1) ^ 2), sq_nonneg ((a + 2) ^ 2),
                          sq_nonneg ((a + 3) ^ 2), sq_nonneg ((a + 4) ^ 2), sq_nonneg ((a + 5) ^ 2),
                          sq_nonneg (a * (a + 1)), sq_nonneg (a * (a + 2)), sq_nonneg (a * (a + 3)),
                          sq_nonneg (a * (a + 4)), sq_nonneg (a * (a + 5)), sq_nonneg ((a + 1) * (a + 2)),
