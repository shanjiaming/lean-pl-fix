    ...
    have h₁ : ∃ (k : ℕ) (p : ℤ), 0 < k ∧ k ≤ n ∧ |(k : ℝ) * α - p| < 1 / (n + 1 : ℝ):=
      ...
      --  · use j - i, round ((j : ℝ) * α) - round ((i : ℝ) * α)
        ...
        have h₂ :
          ...
          have h₃ : ∃ (i j : ℕ), i < j ∧ j ≤ n ∧ |f j - f i| < 1 / (n + 1 : ℝ):= fun k => (↑k : ℝ) * α - (↑(round ((↑k : ℝ) * α)) : ℝ)) : ∃ i j, i < j ∧ j ≤ n ∧ |f j - f i| < 1 / ((↑n : ℝ) + 1) :=
            by
            --  by_contra h
            --  push_neg at h
            have h₄ : ∀ (i j : ℕ), i < j → j ≤ n → 1 / (n + 1 : ℝ) ≤ |f j - f i|:= fun k => (↑k : ℝ) * α - (↑(round ((↑k : ℝ) * α)) : ℝ)) (h : ∀ (i j : ℕ), i < j → j ≤ n → 1 / ((↑n : ℝ) + 1) ≤ |f j - f i|) : ∀ (i j : ℕ), i < j → j ≤ n → 1 / ((↑n : ℝ) + 1) ≤ |f j - f i| :=
              by
              --  intro i j hij hjn
              --  exact h i j hij hjn
              hole
            have h₅ : ∀ k : ℕ, k ≤ n → f k = (k : ℝ) * α - round ((k : ℝ) * α):= fun k => (↑k : ℝ) * α - (↑(round ((↑k : ℝ) * α)) : ℝ)) (h : ∀ (i j : ℕ), i < j → j ≤ n → 1 / ((↑n : ℝ) + 1) ≤ |f j - f i|) (h₄ : ∀ (i j : ℕ), i < j → j ≤ n → 1 / ((↑n : ℝ) + 1) ≤ |f j - f i|) : ∀ k ≤ n, f k = (↑k : ℝ) * α - (↑(round ((↑k : ℝ) * α)) : ℝ) :=
              by
              --  intro k hk
              --  rfl
              hole
