theorem derivative_value_at_pi_over_3 : (2 * (1 / cos (π / 3)) ^ 2 * tan (π / 3)) = 8 * Real.sqrt 3:=
  by
  have h₁ : Real.cos (Real.pi / 3) = 1 / 2:= by -- norm_num [Real.cos_pi_div_three] <;> linarith [Real.pi_pos]
    norm_num
  have h₂ : Real.tan (Real.pi / 3) = Real.sqrt 3:=
    by
    ...
    hole
  have h₃ : (1 / Real.cos (Real.pi / 3)) ^ 2 = 4:= by -- -- rw [h₁] <;> norm_num <;> linarith [Real.pi_pos]
    norm_num
