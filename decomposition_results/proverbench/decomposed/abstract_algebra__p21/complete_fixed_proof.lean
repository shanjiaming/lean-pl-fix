theorem commutative_ring_of_cube_equals_self (h : ∀ a : R, a ^ 3 = a) : ∀ a b : R, a * b = b * a:=
  by
  have h6 : ∀ (a : R), 6 • a = 0:= by
    intro a
    have h₁ : (2 : ℕ) • a * ((2 : ℕ) • a * ((2 : ℕ) • a)) = (2 : ℕ) • a:=
      by
      have h₂ := h ((2 : ℕ) • a)
  ...
  have h2 : ∀ (a b : R), 2 • (a * b) = 0:= by
    ...
    have h₂ : (a - b) ^ 3 = (a - b : R):= by simpa using h (a - b)
      hole
    have h₃ : (a + b) ^ 3 - (a - b) ^ 3 = (a + b : R) - (a - b : R):= by rw [h₁, h₂]
      hole
    ...
    have h₅ : (a + b : R) - (a - b : R) = 2 * b:= by
      ring_nf <;> simp [sub_eq_add_neg, add_assoc, add_comm, add_left_comm, mul_comm, mul_assoc, mul_left_comm] <;> abel_nf
      hole
    have h₆ : 2 * (a ^ 2 * b + a * b * a + b * a ^ 2 + b ^ 3) = 2 * b:= by linarith
      hole
    have h₇ : a ^ 2 * b + a * b * a + b * a ^ 2 + b ^ 3 = b:=
      by
        ...
        have h₁₄ : a ^ 2 = -3 * a:=
          ...
          have h₁₅ : a ^ 2 = -3 * a:= by
            ...
            have h₁₅₂ : a ^ 2 = -3 * a:= by
              rw [← sub_eq_zero]
              have h₁₅₃ : a ^ 2 - (-3 * a) = 0 := by
                simp [sub_eq_add_neg] <;> nlinarith [h6 a, h6 (a ^ 2), h a, h (a + 1), h (a - 1)]
              have h₁₅₃ : a ^ 2 - (-3 * a) = 0:= by simp [sub_eq_add_neg] <;> nlinarith [h6 a, h6 (a ^ 2), h a, h (a + 1), h (a - 1)]
                hole
            have h₁₅₂ : a ^ 2 = -3 * a := by
              rw [← sub_eq_zero]
              have h₁₅₃ : a ^ 2 - (-3 * a) = 0 := by
