  ...
  have h_main : ∃ (a b : ℝ), a < b ∧ (∀ (x : ℝ), x < a → deriv (deriv f) x > 0) ∧ (∀ (x : ℝ), x > b → deriv (deriv f) x > 0) ∧ a = (21 - Real.sqrt 497) / 4 ∧ b = (21 + Real.sqrt 497) / 4 := by
    ...
    have h₁₂ : ∀ x < a, deriv (deriv f) x > 0 := by
      ...
      have h₁₇ : deriv (deriv f) x < 0 := by
        ...
        have h₁₇₃ : deriv (deriv f) x > 0 := by
          ...
          contradiction
        linarith
      -- Use the fact that f''(a - 1) > 0 and the continuity of f'' to derive a contradiction
      have h₁₈ : ContinuousOn (deriv (deriv f)) (Set.Icc x (a - 1)) := by
        have h₁₈₁ : ContinuousOn (deriv (deriv f)) (Set.Icc x (a - 1)) := by
          -- Prove that f'' is continuous on [x, a - 1]
          ...
          have h₁₈₃ : ContinuousOn (deriv (deriv f)) (Set.Icc x (a - 1)) := by
            have h₁₈₄ : ContinuousOn (fun y : ℝ => (50 * y ^ 2 - 525 * y - 175) / ((5 * y ^ 2 + 35) ^ 2)) (Set.Icc x (a - 1)) := by
              ...
              · -- Prove that the numerator is continuous on [x, a - 1]
                apply ContinuousOn.sub
                · apply ContinuousOn.sub
                  · exact continuousOn_const.mul (continuousOn_pow 2)
                  · exact continuousOn_const.mul continuousOn_id
                · exact continuousOn_const
              · -- Prove that the denominator is continuous and non-zero on [x, a - 1]
    ...
    have h₁₅ : ∀ x > b, deriv (deriv f) x > 0 := by
      ...
      have h₂₁ : ContinuousOn (deriv (deriv f)) (Set.Icc (b + 1) x) := by
        have h₂₁₁ : ContinuousOn (deriv (deriv f)) (Set.Icc (b + 1) x) := by
          -- Prove that f'' is continuous on [b + 1, x]
          have h₂₁₂ : ∀ y : ℝ, deriv (deriv f) y = (50 * y ^ 2 - 525 * y - 175) / ((5 * y ^ 2 + 35) ^ 2) := by
            intro y
            rw [f_second_deriv]
            <;> ring_nf
          ...
          have h₂₁₃ : ContinuousOn (deriv (deriv f)) (Set.Icc (b + 1) x) := by
            have h₂₁₄ : ContinuousOn (fun y : ℝ => (50 * y ^ 2 - 525 * y - 175) / ((5 * y ^ 2 + 35) ^ 2)) (Set.Icc (b + 1) x) := by
              apply ContinuousOn.div
              · -- Prove that the numerator is continuous on [b + 1, x]
                apply ContinuousOn.sub
                · apply ContinuousOn.sub
                  · exact continuousOn_const.mul (continuousOn_pow 2)
                  · exact continuousOn_const.mul continuousOn_id
                · exact continuousOn_const
              · -- Prove that the denominator is continuous and non-zero on [b + 1, x]
