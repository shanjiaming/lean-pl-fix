theorem exists_ab_mod_p (p : ℕ) (hp : Nat.Prime p) : ∃ (a b : ℤ), (a ^ 2 + b ^ 2 + 1) % p = 0:=
  by
  have h_main : p = 2 ∨ p ≠ 2:= by
    ...
    --  · exact Or.inr h
    omega
  have h_case_p_eq_2 : p = 2 → ∃ (a b : ℤ), (a ^ 2 + b ^ 2 + 1) % p = 0:=
    by
    --  intro h_p_eq_2
    have h₁ : (1 : ℤ) ^ 2 + (0 : ℤ) ^ 2 + 1 = 2:= by -- norm_num
      norm_num
    have h₂ : ((1 : ℤ) ^ 2 + (0 : ℤ) ^ 2 + 1 : ℤ) % p = 0:= by -- -- rw [h_p_eq_2] <;> norm_num
      hole
    --  refine' ⟨1, 0, _⟩
    --  simpa using h₂
    hole
  have h_case_p_ne_2 : p ≠ 2 → ∃ (a b : ℤ), (a ^ 2 + b ^ 2 + 1) % p = 0:=
    by
    --  intro h_p_ne_2
    --  haveI := Fact.mk hp
    have h₁ : p ≠ 2 := h_p_ne_2
