theorem sum_of_idempotents_zero_implies_zero : e = 0 ∧ g = 0 ∧ f = 0:=
  by
  have hf_eq : f = -e - g:= by
    ...
    have h2 : f = -(e + g):= by
      ...
      exact h4
      hole
    have h3 : f = -e - g:= by rw [h2] <;> ring_nf <;> simp_all [sub_eq_add_neg] <;> linarith
      hole
    exact h3
    hole
  have h_eg_comm : g * e = e * g:= by
    ...
    have h3 : f * f = f:= by apply hf
      hole
    have h4 : g * g = g:= by apply hg
      hole
    have h5 : e * e = e:= by apply he
      hole
    have h6 : g * e = e * g:= by
      calc
        g * e = g * e := rfl
