theorem series_convergence' {b : ℕ → ℝ} (h_pos : ∀ n, b n > 0) (h_lim : Tendsto b atTop (𝓝 (1 / 2))) :
  Summable (fun n => |b n ^ n * cos (n * π) / n|):=
  by
  have h_main : Summable (fun n => (b n) ^ n / n : ℕ → ℝ):=
    by
    have h₁ : ∃ (N : ℕ), ∀ n ≥ N, b n < 3 / 4:=
      by
      have h₂ : Filter.Tendsto b atTop (𝓝 (1 / 2)) := h_lim
      have h₃ : ∀ᶠ (n : ℕ) in atTop, b n ∈ Set.Ioo (1 / 2 - 1 / 4) (1 / 2 + 1 / 4):=
        by
        have h₄ : Filter.Tendsto b atTop (𝓝 (1 / 2)) := h_lim
        have h₅ : (1 / 2 : ℝ) - 1 / 4 < 1 / 2 + 1 / 4:= by norm_num
          hole
        have h₆ : b ⁻¹' Set.Ioo (1 / 2 - 1 / 4) (1 / 2 + 1 / 4) ∈ atTop:=
          by
        ...
        exact h₆
        hole
      have h₇ : ∃ (N : ℕ), ∀ (n : ℕ), n ≥ N → b n ∈ Set.Ioo (1 / 2 - 1 / 4) (1 / 2 + 1 / 4):=
        by
      ...
      hole
    obtain ⟨N, hN⟩ := h₁
    have h₂ : Summable (fun n => (b n) ^ n / n : ℕ → ℝ):=
      by
      have h₃ : ∀ n ≥ N, (b n : ℝ) < 3 / 4:= by
        intro n hn
        exact hN n hn
        hole
      have h₄ : Summable (fun n : ℕ => ((3 / 4 : ℝ) : ℝ) ^ n : ℕ → ℝ):= by apply summable_geometric_of_lt_one <;> norm_num
        hole
      have h₅ : ∀ n ≥ N, (b n : ℝ) ^ n / n ≤ ((3 / 4 : ℝ) : ℝ) ^ n:=
        by
        intro n hn
        have h₅₁ : (b n : ℝ) < 3 / 4 := h₃ n hn
        have h₅₂ : 0 < (b n : ℝ):= by exact_mod_cast h_pos n
          hole
        have h₅₃ : (b n : ℝ) ^ n ≤ ((3 / 4 : ℝ) : ℝ) ^ n:= by exact pow_le_pow_of_le_left (by positivity) (by linarith) n
          hole
        ...
        calc
          ...
          _ ≤ ((3 / 4 : ℝ) : ℝ) ^ n := h₅₃
        hole
      have h₆ : Summable (fun n : ℕ => (b n : ℝ) ^ n / n : ℕ → ℝ):=
        by
    ...
    exact h₂
    hole
  have h_final : Summable (fun n => |b n ^ n * cos (n * π) / n|):=
    by
    have h₁ : (fun n => |b n ^ n * cos (n * π) / n| : ℕ → ℝ) = (fun n => (b n) ^ n / n : ℕ → ℝ):=
      by
      funext n
      have h₂ : |b n ^ n * Real.cos (n * Real.pi) / (n : ℝ)| = (b n) ^ n / n:=
        by
        have h₃ : Real.cos (n * Real.pi) = (-1 : ℝ) ^ n:=
          by
        ...
        have h₅ : (b n : ℝ) > 0:= by exact_mod_cast h₄
          hole
        have h₆ : |(b n : ℝ) ^ n * ((-1 : ℝ) : ℝ) ^ n / (n : ℝ)| = (b n : ℝ) ^ n / n:=
          by
          have h₇ : |(b n : ℝ) ^ n * ((-1 : ℝ) : ℝ) ^ n / (n : ℝ)| = |(b n : ℝ) ^ n| * |((-1 : ℝ) : ℝ) ^ n| / |(n : ℝ)|:= by
            ...
            hole
          rw [h₇]
          have h₈ : |(b n : ℝ) ^ n| = (b n : ℝ) ^ n:= by exact abs_of_nonneg (by positivity)
            hole
          have h₉ : |((-1 : ℝ) : ℝ) ^ n| = 1:=
            by
            have h₁₀ : ((-1 : ℝ) : ℝ) ^ n = 1 ∨ ((-1 : ℝ) : ℝ) ^ n = -1:=
              by
