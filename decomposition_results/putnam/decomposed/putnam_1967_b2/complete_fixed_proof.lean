theorem putnam_1967_b2 (p r A B C α β γ : ℝ) (prbound : 0 ≤ p ∧ p ≤ 1 ∧ 0 ≤ r ∧ r ≤ 1)
  ...
  have hA : A = p ^ 2:= by
    ...
    have h₇ := id1 2 (-1)
    have h₈ := id1 (-1) 2
    --  --  norm_num at h₁ h₂ h₃ h₄ h₅ h₆ h₇ h₈ ⊢ <;> nlinarith [prbound.1, prbound.2.1, prbound.2.2.1, prbound.2.2.2]
    linarith
  ...
  have hC : C = (1 - p) ^ 2:= by
    ...
    have h₈ := id1 2 (-1)
    have h₉ := id1 (-1) 2
    --  --  norm_num at h₁ h₂ h₃ h₄ h₅ h₆ h₇ h₈ h₉ ⊢ <;> nlinarith [prbound.1, prbound.2.1, prbound.2.2.1, prbound.2.2.2, hA, hB]
    linarith
  ...
  have h_max_αβγ : max (max α β) γ ≥ 4 / 9:=
    ...
    have h₉ : α + γ < 8 / 9:= by -- nlinarith
      linarith
    have h₃ : max (max α β) γ < 4 / 9:= by -- -- -- -- simp_all [max_lt_iff] <;> (try norm_num) <;> (try linarith) <;> (try nlinarith)
      hole
  have h_max_αβγ : max (max α β) γ ≥ 4 / 9 :=
    by
    ...
    have h₃ : max (max A B) C ≥ 4 / 9:= by
      ...
      --      linarith
        ·
          ...
          have h₆' : max (max A B) C ≥ A:= by -- -- apply le_trans _ (le_max_left (max A B) C) <;> simp [h₆] <;> apply le_max_left
            norm_num
