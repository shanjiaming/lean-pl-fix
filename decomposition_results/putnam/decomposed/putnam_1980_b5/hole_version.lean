  ...
  have h_main : (t ≤ 1) ↔ (∀ f ∈ S t, ∀ g ∈ S t, f * g ∈ S t) := by
    ...
    · 
      ...
      have h₂ : S t = {f : ℝ → ℝ | (∀ x ∈ T, f x ≥ 0) ∧ StrictMonoOn f T ∧ IsConvex f ∧ ContinuousOn f T ∧ P t f} := by
        hole_1
      rw [h₂] at hf hg ⊢
      have h₃ : (∀ x ∈ T, f x ≥ 0) ∧ StrictMonoOn f T ∧ IsConvex f ∧ ContinuousOn f T ∧ P t f := by hole_2
      have h₄ : (∀ x ∈ T, g x ≥ 0) ∧ StrictMonoOn g T ∧ IsConvex g ∧ ContinuousOn g T ∧ P t g := by hole_3
      have h₅ : (∀ x ∈ T, (f * g) x ≥ 0) := by
        intro x hx
        have h₅₁ : f x ≥ 0 := h₃.1 x hx
        have h₅₂ : g x ≥ 0 := h₄.1 x hx
        have h₅₃ : (f * g) x = f x * g x := by hole_5
        hole_4
      have h₆ : StrictMonoOn (f * g) T := by
        ...
        have h₇₀ : (f * g) x = f x * g x := by hole_7
        have h₇₁ : (f * g) y = f y * g y := by hole_8
        rw [h₇₀, h₇₁]
        have h₇₂ : f x ≥ 0 := h₆₃ x h₆₅
        have h₇₃ : f y ≥ 0 := h₆₃ y h₆₆
        have h₇₄ : g x ≥ 0 := h₆₄ x h₆₅
      ...
      have h₇ : IsConvex (f * g) := by
        ...
        have h₇₅ : IsConvex (f * g) := by
          ...
          have h₇₈ : (f * g) (s * u + (1 - s) * v) = f (s * u + (1 - s) * v) * g (s * u + (1 - s) * v) := by hole_13
