...
: ∃ f : ℝ → Set ℕ+, ∀ a b : ℝ, a < b → f a ⊂ f b := by
  have h_main : ∃ (q : ℕ+ → ℚ), Function.Bijective q := by
    ...
    have h₁ : ∃ (f : ℕ+ → ℚ), Function.Bijective f := by
      ...
      have h₂ : Denumerable ℚ := by hole_4
      
      have h₃ : Denumerable ℕ+ := by hole_5
  ...
  have h_final : ∃ f : ℝ → Set ℕ+, ∀ a b : ℝ, a < b → f a ⊂ f b := by
    ...
    have h₂ : {n : ℕ+ | (q n : ℝ) < a} ≠ {n : ℕ+ | (q n : ℝ) < b} := by
      ...
      have h₅ : m ∈ {n : ℕ+ | (q n : ℝ) < b} := by
        ...
        have h₅₂ : (r : ℝ) < b := hr₂
        have h₅₃ : (q m : ℝ) < b := by hole_14
        hole_12
      have h₆ : m ∉ {n : ℕ+ | (q n : ℝ) < a} := by
        have h₆₁ : (q m : ℝ) = (r : ℝ) := by hole_16
        have h₆₂ : a < (r : ℝ) := hr₁
        have h₆₃ : ¬(q m : ℝ) < a := by
          intro h
          have h₆₄ : (q m : ℝ) < a := h
          have h₆₅ : (q m : ℝ) = (r : ℝ) := by hole_18
          hole_17
        hole_15
      hole_9
    have h₃ : {n : ℕ+ | (q n : ℝ) < a} ⊂ {n : ℕ+ | (q n : ℝ) < b} := by
