...
: (∃ F : Fin 3 → ((Fin 3 → ℝ) → ℝ), Fprop1 F ∧ Fprop2 F ∧ Fprop3 F) ↔ ((False) : Prop ) := by
  have h_main : ¬ (∃ F : Fin 3 → ((Fin 3 → ℝ) → ℝ), Fprop1 F ∧ Fprop2 F ∧ Fprop3 F) := by
    ...
    have h₉ := h₃ 1 (-1)
    have h₁₀ := h₃ (-1) 1
    norm_num [hG, hvec2, hvec3, Function.funext_iff, Fin.forall_fin_succ] at h₄ h₅ h₆ h₇ h₈ h₉ h₁₀
    <;>
    (try norm_num) <;>
    (try contradiction) <;>
    (try linarith) <;>
    (try
      {
        ...
        have h₂₁ := h₂ (vec3 0 2 1)
        have h₂₂ := h₂ (vec3 1 1 1)
        norm_num [hG, hvec2, hvec3, Function.funext_iff, Fin.forall_fin_succ, hpartderiv, hvrepl, hcontdiffv] at h₁₁ h₁₂ h₁₃ h₁₄ h₁₅ h₁₆ h₁₇ h₁₈ h₁₉ h₂₀ h₂₁ h₂₂
        <;>
        (try norm_num) <;>
        (try contradiction) <;>
        (try linarith)
