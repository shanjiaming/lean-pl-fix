theorem putnam_2014_b4 (n : ℕ) (P : Polynomial ℂ) (npos : n > 0)
  (Px : P.degree = n ∧ ∀ k ∈ Set.Icc 0 n, P.coeff k = 2 ^ (k * (n - k))) : ∀ r ∈ P.roots, r.im = 0:=
  by
  have h₁ : ∀ (k : ℕ), k ∈ Set.Icc 0 n → (P.coeff k : ℂ).im = 0:=
    by
    --  intro k hk
    have h₂ : P.coeff k = (2 : ℂ) ^ (k * (n - k)):=
      by
      have h₃ : P.coeff k = (2 : ℂ) ^ (k * (n - k)):= by -- simpa using Px.2 k hk
        hole
      --  exact h₃
      simpa
    --  rw [h₂]
  ...
  have h₃ : ∀ (r : ℂ), r ∈ P.roots → r.im = 0:= by
    --  intro r hr
    have h₄ : P.eval r = 0:= by -- apply Polynomial.mem_roots h₂ |>.mp hr
      hole
