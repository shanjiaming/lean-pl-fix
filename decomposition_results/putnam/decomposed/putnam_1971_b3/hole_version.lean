  ...
  have h_main : False := by
    ...
    have h₃ : ∃ (t : ℝ), t ≥ T ∧ Nat.floor t = 2 * (Nat.floor (t - T)) := by
      ...
      have h₅ : Nat.floor (2 * T : ℝ) = 2 * Nat.floor T := by
        ...
        have h₅₃ : (2 * (Nat.floor T : ℝ) : ℝ) ≤ 2 * T := by hole_7
        have h₅₄ : 2 * T < (2 * (Nat.floor T : ℝ) : ℝ) + 2 := by hole_8
        have h₅₅ : Nat.floor (2 * T : ℝ) = 2 * Nat.floor T := by
          hole_9
        hole_6
      have h₆ : Nat.floor ((2 * T : ℝ) - T) = Nat.floor T := by
        
        have h₆₁ : (2 * T : ℝ) - T = T := by hole_11
        hole_10
    ...
    have h₆ : (T : ℝ) > 0 := by hole_13
    
    have h₇ : False := by
      by_contra h₇
      
      have h₈ : (Nat.floor t : ℝ) ≤ t := by hole_15
      have h₉ : t < (Nat.floor t : ℝ) + 1 := by
        hole_16
      have h₁₀ : (Nat.floor (t - T) : ℝ) ≤ t - T := by hole_17
      have h₁₁ : t - T < (Nat.floor (t - T) : ℝ) + 1 := by
        hole_18
      have h₁₂ : (Nat.floor t : ℝ) = 2 * (Nat.floor (t - T) : ℝ) := by
        hole_19
      have h₁₃ : (Nat.floor t : ℝ) ≤ t := by hole_20
      have h₁₄ : t < (Nat.floor t : ℝ) + 1 := by
        hole_21
      have h₁₅ : (Nat.floor (t - T) : ℝ) ≤ t - T := by hole_22
      have h₁₆ : t - T < (Nat.floor (t - T) : ℝ) + 1 := by
        hole_23
      have h₁₇ : (Nat.floor t : ℝ) = 2 * (Nat.floor (t - T) : ℝ) := by
        hole_24
