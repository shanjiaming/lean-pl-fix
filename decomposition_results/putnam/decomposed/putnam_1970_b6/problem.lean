...
: Cospherical S := by
    ...
    have h₃ : Function.Injective L := by
      ...
      have h₆ : S.ncard < 4 := by
        have h₇ : S.ncard ≤ 3 := by
          have h₈ : S = {L i | i : ZMod 4} := S_def
          have h₉ : S.ncard ≤ 3 := by
            -- Use the fact that the cardinality of the image is less than or equal to the cardinality of the domain minus the number of collisions
            have h₁₀ : S.ncard ≤ (Finset.univ : Finset (ZMod 4)).card := by
              -- Use the fact that the cardinality of the image is less than or equal to the cardinality of the domain
              have h₁₁ : S.ncard ≤ (Finset.univ : Finset (ZMod 4)).card := by
                calc
                  S.ncard ≤ (Set.image L (Set.univ : Set (ZMod 4))).ncard := by
                    have h₁₂ : S = Set.image L (Set.univ : Set (ZMod 4)) := by
                      rw [h₂]
                      <;> simp [Set.ext_iff]
                      <;> aesop
                    rw [h₁₂]
                  _ ≤ (Finset.univ : Finset (ZMod 4)).card := by
                    -- Use the fact that the cardinality of the image is less than or equal to the cardinality of the domain
                    have h₁₃ : (Set.image L (Set.univ : Set (ZMod 4))).ncard ≤ (Finset.univ : Finset (ZMod 4)).card := by
                      have h₁₄ : (Set.image L (Set.univ : Set (ZMod 4))).ncard ≤ (Finset.image L (Finset.univ : Finset (ZMod 4))).card := by
    ...
    have h₄ : ¬Collinear ℝ ({L i, L j, L k} : Set (EuclideanSpace ℝ (Fin 2))) := by
      ...
      have h₆ : ({L i, L j, L k} : Set (EuclideanSpace ℝ (Fin 2))).ncard = 3 := by
        ...
        have h₁₀ : ({L i, L j, L k} : Set (EuclideanSpace ℝ (Fin 2))).ncard = 3 := by
          ...
          have h₁₃ : L j ≠ L k := h₉
          have h₁₄ : ({L i, L j, L k} : Set (EuclideanSpace ℝ (Fin 2))).ncard = 3 := by
            norm_num [Set.ncard_insert_of_not_mem, Set.mem_singleton_iff, h₇, h₈, h₉]
            <;>
            (try decide) <;>
  ...
  have h_interior_nonempty : (interior (convexHull ℝ S)).Nonempty := by
    have h₁ : (interior (convexHull ℝ S)).Nonempty := by
      by_contra h
      -- If the interior of the convex hull is empty, then the volume of the convex hull is zero.
      ...
      have h₃ : (MeasureTheory.volume (convexHull ℝ S)).toReal = 0 := by
        ...
        have h₆ : MeasureTheory.volume (convexHull ℝ S : Set (EuclideanSpace ℝ (Fin 2))) = 0 := by
          have h₆₁ : MeasureTheory.volume (frontier (convexHull ℝ S) : Set (EuclideanSpace ℝ (Fin 2))) = 0 := by
            -- The volume of the frontier of the convex hull is zero because it is a lower-dimensional set.
            apply isClosed_convexHull.measure_zero_frontier
            <;> aesop
          rw [h₅] at *
          exact h₆₁
  ...
  have h_ex_intersection : ∃ (i : ZMod 4), (segment ℝ (L i) (L (i + 1))) ∩ interior (convexHull ℝ S) ≠ ∅ := by
    by_contra! h
    -- We aim to show that if all segments do not intersect the interior, then the convex hull has no volume, which contradicts h_vol_pos.
    have h₁ : ∀ (i : ZMod 4), (segment ℝ (L i) (L (i + 1))) ∩ interior (convexHull ℝ S) = ∅ := h
    -- This is obtained by assuming the negation and applying h.
    have h₂ : (interior (convexHull ℝ S)).Nonempty := h_interior_nonempty
    obtain ⟨x, hx⟩ := h₂
    -- We know the interior is nonempty, so we take an arbitrary point x in the interior.
    have h₃ : x ∈ convexHull ℝ S := interior_subset hx
    -- Since x is in the interior, it is also in the convex hull.
    have h₄ : x ∈ convexHull ℝ (Set.range L) := by
      -- We need to show that x is in the convex hull of the set of all L i's.
      have h₅ : S = Set.range L := by
        -- We show that S is the range of L.
        rw [S_def]
        <;> simp [Set.ext_iff]
        <;> aesop
      rw [h₅] at h₃
      exact h₃
    -- Since x is in the convex hull of the range of L, we can express x as a convex combination of points in the range of L.
    have h₅ : x ∈ convexHull ℝ (Set.range L) := h₄
    -- We use Carathéodory's theorem to get a specific representation of x.
    have h₆ : x ∈ convexHull ℝ (Set.range L) := h₅
    -- We use the fact that L is injective to get a canonical form for x.
    have h₇ : x ∈ convexHull ℝ (Set.range L) := h₆
    -- We use the injectivity of L to get a canonical form for x.
    have h₈ : x ∈ convexHull ℝ (Set.range L) := h₇
    -- We use the fact that L is injective to get a canonical form for x.
    have h₉ : x ∈ convexHull ℝ (Set.range L) := h₈
    -- We use the fact that L is injective to get a canonical form for x.
    have h₁₀ : x ∈ convexHull ℝ (Set.range L) := h₉
    -- We use the fact that L is injective to get a canonical form for x.
    -- We will show that this leads to a contradiction with h₁.
    have h₁₁ : False := by
      -- We will show that this leads to a contradiction with h₁.
      classical
      -- We will show that this leads to a contradiction with h₁.
      ...
      have h₁₉ := h₁₅ x
      -- We will show that this leads to a contradiction with h₁.
      by_cases h₂₀ : x ∈ segment ℝ (L 0) (L 1)
      <;> by_cases h₂₁ : x ∈ segment ℝ (L 1) (L 2)
