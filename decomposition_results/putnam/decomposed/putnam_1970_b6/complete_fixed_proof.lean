...
: Cospherical S := by
  ...
  have h_vol_pos : (MeasureTheory.volume (convexHull ℝ S)).toReal > 0 := by
    ...
    have h₂ : Real.sqrt (dist (L 0) (L 1) * dist (L 1) (L 2) * dist (L 2) (L 3) * dist (L 3) (L 0)) > 0 := by
      ...
      have h₃ : 0 < dist (L 0) (L 1) := by admit
      have h₄ : 0 < dist (L 1) (L 2) := by admit
      have h₅ : 0 < dist (L 2) (L 3) := by admit
      have h₆ : 0 < dist (L 3) (L 0) := by admit
      have h₇ : 0 < dist (L 0) (L 1) * dist (L 1) (L 2) := by admit
      have h₈ : 0 < dist (L 1) (L 2) * dist (L 2) (L 3) := by admit
      have h₉ : 0 < dist (L 0) (L 1) * dist (L 1) (L 2) * dist (L 2) (L 3) := by admit
      have h₁₀ : 0 < dist (L 0) (L 1) * dist (L 1) (L 2) * dist (L 2) (L 3) * dist (L 3) (L 0) := by admit
      admit
    admit
