theorem h_main (n k : ℕ) (hnk : n > 0 ∧ k > 0) : Odd {s | klimited k n s}.ncard ↔ n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1] :=
  by
  have h₁ : n > 0 := hnk.1
  have h₂ : k > 0 := hnk.2
  have h₃ : 2 * k + 1 > 0 := by sorry
  --  cases' le_or_lt n (k + 1) with h h
  ·
    have h₄ : n ≤ k + 1 := h
    have h₅ : n ≤ k + 1 := h₄
    have h₆ : ∀ (s : Equiv.Perm (Fin n)), klimited k n s :=
      by
      intro s
      intro i
      have h₇ : |((s i) : ℤ) - i| ≤ k :=
        ...
        exact h₃₀
      exact h₇
    have h₃₁ : {s : Equiv.Perm (Fin n) | klimited k n s} = Set.univ := by sorry
    have h₃₂ : Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s} = Set.ncard (Set.univ : Set (Equiv.Perm (Fin n))) :=
      by rw [h₃₁]
    have h₃₃ : Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s} = Nat.factorial n := by sorry
    have h₃₄ :
  ...
  ·
    ...
    have h₅ :
      ...
      by
      have h₅₁ : n > k + 1 := h₄
      have h₅₂ : ∀ (s : Equiv.Perm (Fin n)), klimited k n s → s = 1 :=
        by
        intro s hs
        ...
        have h₅₄ : s = 1 := by
          ...
          have h₅₆ : (s i : ℕ) = i := by
            ...
            have h₅₉ : (i : ℕ) < n := i.is_lt
            have h₆₀ : |((s i) : ℤ) - i| ≤ k := h₅₅
            have h₆₁ : ((s i) : ℤ) - i ≤ k := by linarith [abs_le.mp h₆₀]
            have h₆₂ : ((s i) : ℤ) - i ≥ -k := by linarith [abs_le.mp h₆₀]
            ...
            have h₆₅ : (s i : ℕ) ≤ i + k := by
              ...
              have h₆₈ : (s i : ℕ) ≤ i + k := by
                ...
                have h₇₁ : (s i : ℤ) > (i : ℤ) + k :=
                  by
                  have h₇₂ : (s i : ℤ) > (i : ℤ) + k := by norm_cast at h₇₀ ⊢ <;> omega
                  exact h₇₂
                linarith
              exact h₆₈
            have h₆₆ : (s i : ℕ) ≥ i - k := by
          ...
          simp [h₅₇]
        exact h₅₄
      have h₅₈ : {s : Equiv.Perm (Fin n) | klimited k n s} = { 1 } :=
        by
      ...
      have h₅₉ : Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s} = 1 :=
        by
        rw [h₅₈]
        simp [Set.ncard_singleton]
      have h₆₀ : Odd (Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s}) := by
        rw [h₅₉] <;> simp [Nat.odd_iff_not_even, parity_simps]
      have h₆₁ : n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1] :=
        by
        exfalso
        have h₆₂ := h₅₂ (Equiv.swap (⟨0, by omega⟩ : Fin n) ⟨1, by omega⟩)
        have h₆₃ : klimited k n (Equiv.swap (⟨0, by omega⟩ : Fin n) ⟨1, by omega⟩) → False :=
          by
          intro h₆₄
          have h₆₅ := h₆₄
      ...
      simp_all
    have h₆ : ∀ (s : Equiv.Perm (Fin n)), klimited k n s := by sorry
have h_main :
  Odd (Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s}) ↔ (n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1]) :=
  by
  have h₁ : n > 0 := hnk.1
  ...
  ·
    ...
    have h₆ : ∀ (s : Equiv.Perm (Fin n)), klimited k n s :=
      ...
      have h₇ : |((s i) : ℤ) - i| ≤ k :=
        ...
        exact h₃₀
      exact h₇
    have h₃₁ : {s : Equiv.Perm (Fin n) | klimited k n s} = Set.univ :=
      by
    ...
    have h₃₂ : Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s} = Set.ncard (Set.univ : Set (Equiv.Perm (Fin n))) :=
      by rw [h₃₁]
    have h₃₃ : Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s} = Nat.factorial n :=
      by
      rw [h₃₂]
      have h₃₄ : Set.ncard (Set.univ : Set (Equiv.Perm (Fin n))) = Nat.factorial n := by
        rw [Set.ncard_univ] <;> simp [Fintype.card_perm, Nat.factorial] <;> simp_all [Nat.factorial] <;> norm_num <;>
          aesop
    ...
    have h₃₄ :
      ...
      have h₃₉ : n > 0 := h₁
      have h₄₀ : k > 0 := h₂
      have h₄₁ : n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1] → n = 1 :=
        by
        intro h₄₂
      ...
      have h₄₂ : n = 1 → (Odd (Nat.factorial n) ↔ (n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1])) :=
        ...
        have h₄₅ : n ≡ 1 [MOD 2 * k + 1] := by
          rw [h₄₄] <;> simp [Nat.ModEq, Nat.add_mod, Nat.mul_mod, Nat.mod_mod] <;> omega
      ...
      have h₄₃ : n ≠ 1 → ¬(Odd (Nat.factorial n)) := by
        ...
        have h₄₇ : Even (Nat.factorial n) := by
          have h₄₈ : n ≥ 2 := by omega
          have h₄₉ : Even (Nat.factorial n) :=
            by
            have h₅₀ : Even (Nat.factorial n) := by
              rw [even_iff_two_dvd]
              have h₅₁ : 2 ∣ Nat.factorial n := by
                have h₅₂ : 2 ≤ n := by omega
                have h₅₃ : 2 ∣ Nat.factorial n := by exact Nat.dvd_factorial (by omega) (by omega)
                exact h₅₃
              exact h₅₁
            exact h₅₀
          exact h₄₉
        have h₄₈ : ¬Odd (Nat.factorial n) := by
          ...
          exact h₄₉ h₄₇
        exact h₄₈
      have h₄₄ : n ≠ 1 → ¬(n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1]) :=
        by
        intro h₄₅
        have h₄₆ : n ≠ 1 := h₄₅
        have h₄₇ : n > 1 := by
      ...
      by_cases h₄₅ : n = 1
      ·
        have h₄₆ : Odd (Nat.factorial n) ↔ (n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1]) := by apply h₄₂ <;> simp_all
        cases' h₄₆ with h₄₇ h₄₇ <;> simp_all <;> try omega <;> try aesop
      ·
        have h₄₆ : ¬(Odd (Nat.factorial n)) := by apply h₄₃ <;> simp_all
        have h₄₇ : ¬(n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1]) := by apply h₄₄ <;> simp_all
        simp_all <;> aesop
    exact h₃₄
  ·
    have h₄ : n > k + 1 := by omega
    have h₅ :
      ...
      have h₆₁ : n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1] :=
        ...
        exact h₆₃ (by simpa using h₆₂)
      simp_all
    have h₃₂ : Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s} = Set.ncard (Set.univ : Set (Equiv.Perm (Fin n))) := by sorry
have h_main :
  Odd (Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s}) ↔ (n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1]) :=
  by
  have h₁ : n > 0 := hnk.1
  ...
  ·
    ...
    have h₅ :
      ...
      have h₆₁ : n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1] :=
        ...
        exact h₆₃ (by simpa using h₆₂)
      simp_all
    have h₅ : Odd (Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s}) ↔ (n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1]) := by sorry
have h_main :
  Odd (Set.ncard {s : Equiv.Perm (Fin n) | klimited k n s}) ↔ (n ≡ 0 [MOD 2 * k + 1] ∨ n ≡ 1 [MOD 2 * k + 1]) :=
  by
  have h₁ : n > 0 := hnk.1
