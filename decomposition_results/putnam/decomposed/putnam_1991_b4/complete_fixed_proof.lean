theorem putnam_1991_b4 (p : ℕ) (podd : Odd p) (pprime : Prime p) :
  (∑ j : Fin (p + 1), (p.choose j) * ((p + j).choose j)) ≡ (2 ^ p + 1) [MOD (p ^ 2)]:=
  by
  have h_main : (∑ j : Fin (p + 1), (p.choose j) * ((p + j).choose j)) ≡ (2 ^ p + 1) [MOD (p ^ 2)]:=
    by
    ...
    --  rw [h₅]
    have h₆ : ∑ j in Finset.range (p + 1), (Nat.choose p j) * (Nat.choose (p + j) j) ≡ (2 ^ p + 1) [MOD (p ^ 2)]:=
      by
      have h₇ : p = 3:= by
        have h₇₁ := h₁.two_le
        have h₇₂ := h₁.one_lt
