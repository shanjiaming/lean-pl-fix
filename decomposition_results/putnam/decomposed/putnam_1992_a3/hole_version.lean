  ...
  have h_main : S = (if Odd m then ∅ else {(m + 1, 2 ^ (m / 2), 2 ^ (m / 2))}) := by
    ...
    · 
      have h₁ : S = ∅ := by
        ...
        have h₁₂ : x ^ 2 + y ^ 2 > 0 := by hole_4
        have h₁₃ : x * y > 0 := by hole_5
        
        have h₁₄ : (x ^ 2 + y ^ 2) ^ m > 0 := by hole_6
        have h₁₅ : (x * y) ^ n > 0 := by hole_7
    ...
    · 
      ...
      have h₂ : S = {(m + 1, 2 ^ (m / 2), 2 ^ (m / 2))} := by
        ...
        · 
          ...
          have h₁₂ : m = 2 * k := hk
          have h₁₃ : (x ^ 2 + y ^ 2) ^ m = (x * y) ^ n := h₅
          have h₁₄ : x ^ 2 + y ^ 2 > 0 := by hole_17
          have h₁₅ : x * y > 0 := by hole_18
          
          have h₁₆ : (x ^ 2 + y ^ 2) ^ m > 0 := by hole_19
          have h₁₇ : (x * y) ^ n > 0 := by hole_20
