  ...
  have h_main_min_distance : ∀ (A B C : EuclideanSpace ℝ (Fin 2)), ABCall A B C → (3 : ℝ) ≤ dist A B := by
    ...
    have h₇ : (3 : ℝ) ≤ dist A B := by
      have h₈ : dist A B ≥ 3 := by
        ...
        by_contra h
        have h₉ : dist A B < 3 := by hole_10
        
        have h₁₀ : (dist A B : ℝ) ≥ 0 := by hole_11
        have h₁₁ : (dist A B : ℝ) < 3 := by hole_12
        have h₁₂ : (dist A B : ℝ) = round (dist A B) := by hole_13
        have h₁₃ : (round (dist A B) : ℝ) = dist A B := by hole_14
        have h₁₄ : (round (dist A B) : ℤ) = round (dist A B) := by hole_15
        have h₁₅ : (dist A B : ℝ) ≥ 1 := by
          
          by_contra h₁₅
          have h₁₆ : (dist A B : ℝ) < 1 := by hole_16
          have h₁₇ : A = B := by
            ...
            have h₁₈ : A = B := by
              ...
              have h₂₆ : dist A B ≥ 0 := by hole_17
              
              have h₂₇ : A = B := by
                
                have h₂₈ : (dist A B : ℝ) < 1 := by hole_18
                ...
                have h₂₉ : A = B := by
                  ...
                  have h₃₀ : ∀ i : Fin 2, A i = B i := by
                    ...
                    have h₃₃ : round (A i) = round (B i) := by
                      ...
                      have h₃₄ : (dist A B : ℝ) < 1 := by hole_19
                      
                      have h₃₅ : (A i - B i : ℝ) ^ 2 < 1 := by
                        
                        have h₃₆ : (dist A B : ℝ) ^ 2 < 1 := by hole_21
