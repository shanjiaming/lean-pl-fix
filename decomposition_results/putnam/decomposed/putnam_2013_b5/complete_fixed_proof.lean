theorem putnam_2013_b5 (n : ℕ) (hn : n ≥ 1) (k : Set.Icc 1 n) (fiter : (Set.Icc 1 n → Set.Icc 1 n) → Prop)
  (hfiter : ∀ f, fiter f ↔ ∀ x : Set.Icc 1 n, ∃ j : ℕ, f^[j] x ≤ k) : {f | fiter f}.encard = k * n ^ (n - 1):=
  by
  have h1 : False:= by
    ...
    have h6 : ¬(∀ (f : Set.Icc 1 n → Set.Icc 1 n), fiter f ↔ ∀ (x : Set.Icc 1 n), ∃ (j : ℕ), f^[j] x ≤ k):=
      ...
      have h18 : ¬fiter (fun x => x):= by
        ...
        --  exact h12 h20
        hole
      have h10 : (∀ (x : Set.Icc 1 n), ∃ (j : ℕ), (fun x => x : Set.Icc 1 n → Set.Icc 1 n)^[j] x ≤ k) → fiter (fun x => x):=
        by
        --  intro h11
        have h12 := h8
        --  aesop
