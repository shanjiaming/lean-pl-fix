...
: (∃ m ∈ T, ∃ n ∈ T, m ≠ n ∧ m + n = 104) := by
  have h_main : ∃ m ∈ T, ∃ n ∈ T, m ≠ n ∧ m + n = 104 := by
    ...
    have h₄ : T ⊆ ({1, 52} ∪ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({3 * j + 1} : Set ℤ)) ∪ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({103 - 3 * j} : Set ℤ))) := by
      ...
      have h₆ : 0 ≤ j := hj₀
      have h₇ : j ≤ 33 := hj₁
      have h₈ : j ≥ 0 := by hole_2
      have h₉ : j ≤ 33 := by hole_3
      ...
      have h₁₀ : j = 0 ∨ j = 17 ∨ (1 ≤ j ∧ j ≤ 16) ∨ (18 ≤ j ∧ j ≤ 33) := by
        ...
        · by_cases h₁₁ : j = 17
          ...
          · by_cases h₁₂ : j ≤ 16
            · have h₁₃ : 1 ≤ j := by
                by_contra h₁₄
                have h₁₅ : j ≤ 0 := by hole_4
                have h₁₆ : j = 0 := by hole_5
                hole_6
              exact Or.inr (Or.inr (Or.inl ⟨h₁₃, h₁₂⟩))
            · have h₁₃ : 18 ≤ j := by
                by_contra h₁₄
                have h₁₅ : j ≤ 17 := by hole_7
                have h₁₆ : j = 17 := by
                  by_contra h₁₇
                  have h₁₈ : j < 17 := by
                    hole_8
                  have h₁₉ : j ≤ 16 := by hole_9
                  hole_10
                hole_11
              have h₁₄ : j ≤ 33 := by hole_12
              hole_13
      ...
      · 
        ...
        <;> aesop
      · 
        have h₁₃ : 1 ≤ (j : ℤ) := by hole_14
        have h₁₄ : (j : ℤ) ≤ 16 := by hole_15
        have h₁₅ : (3 * j + 1 : ℤ) ∈ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({3 * j + 1} : Set ℤ)) := by
          hole_16
        simp_all [Set.mem_union, Set.mem_singleton_iff]
        <;> aesop
      · 
        have h₁₃ : 18 ≤ (j : ℤ) := by hole_17
        have h₁₄ : (j : ℤ) ≤ 33 := by hole_18
        have h₁₅ : (3 * j + 1 : ℤ) ∈ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({103 - 3 * j} : Set ℤ)) := by
          have h₁₆ : 1 ≤ (34 - j : ℤ) := by
            have h₁₇ : (j : ℤ) ≤ 33 := by hole_19
            hole_20
          have h₁₇ : (34 - j : ℤ) ≤ 16 := by
            have h₁₈ : 18 ≤ (j : ℤ) := by hole_21
            hole_22
      ...
      have h₅₃ : (({1, 52} ∪ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({3 * j + 1} : Set ℤ)) ∪ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({103 - 3 * j} : Set ℤ)) : Set ℤ)).ncard ≤ 18 := by
        ...
        have h₅₄ : (({1, 52} ∪ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({3 * j + 1} : Set ℤ)) ∪ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({103 - 3 * j} : Set ℤ)) : Set ℤ)) ⊆ (Set.Icc 1 103 : Set ℤ) := by
          ...
          · 
            have h₅₅ : 1 ≤ (j : ℤ) := hj.1
            have h₅₆ : (j : ℤ) ≤ 16 := hj.2
            have h₅₇ : x = 3 * j + 1 := by hole_28
            rw [h₅₇]
            have h₅₈ : (1 : ℤ) ≤ 3 * j + 1 := by hole_29
            have h₅₉ : (3 * j + 1 : ℤ) ≤ 103 := by
              have h₅₁₀ : (j : ℤ) ≤ 16 := by hole_30
              hole_31
            exact ⟨h₅₈, h₅₉⟩
          · 
            have h₅₅ : 1 ≤ (j : ℤ) := hj.1
            have h₅₆ : (j : ℤ) ≤ 16 := hj.2
            have h₅₇ : x = 103 - 3 * j := by hole_32
            rw [h₅₇]
            have h₅₈ : (1 : ℤ) ≤ 103 - 3 * j := by
              have h₅₉ : (j : ℤ) ≤ 16 := by hole_33
              hole_34
            have h₅₉ : (103 - 3 * j : ℤ) ≤ 103 := by
              have h₅₁₀ : 1 ≤ (j : ℤ) := by hole_35
              hole_36
            hole_37
        have h₅₅ : (({1, 52} ∪ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({3 * j + 1} : Set ℤ)) ∪ (⋃ (j : ℤ) (_ : 1 ≤ j ∧ j ≤ 16), ({103 - 3 * j} : Set ℤ)) : Set ℤ)).ncard ≤ 18 := by
