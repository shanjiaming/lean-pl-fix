theorem putnam_1990_b2 (x z : ℝ) (P : ℕ → ℝ) (xlt1 : |x| < 1) (zgt1 : |z| > 1)
  (hP : ∀ j ≥ 1, P j = (∏ i : Fin j, (1 - z * x ^ (i : ℕ))) / (∏ i : Set.Icc 1 j, (z - x ^ (i : ℕ)))) :
  1 + (∑' j : Set.Ici 1, (1 + x ^ (j : ℕ)) * P j) = 0:=
  by
  have h_main : (∑' j : Set.Ici 1, (1 + x ^ (j : ℕ)) * P j) = -1:=
    by
    have h₁ : ∀ (n : ℕ), n ≥ 1 → P n = (∏ i : Fin n, (1 - z * x ^ (i : ℕ))) / (∏ i : Set.Icc 1 n, (z - x ^ (i : ℕ))):=
      by
      --  intro n hn
      --  exact hP n hn
      hole
    have h₂ : ∀ (n : ℕ), n ≥ 1 → P n = 0:= by
      ...
      have h₄ : ∏ i : Fin n, (1 - z * x ^ (i : ℕ)) = 0:=
        ...
        --  exact h₆
        hole
      have h₅ : ∏ i : Set.Icc 1 n, (z - x ^ (i : ℕ)) ≠ 0:=
        by
        have h₆ : ∀ (i : ℕ), i ∈ Set.Icc 1 n → z - x ^ i ≠ 0:=
          by
          --  intro i hi
      ...
      --  exact h₆
      hole
    have h₃ : (∑' j : Set.Ici 1, (1 + x ^ (j : ℕ)) * P j) = -1:=
      by
      have h₄ : ∀ (j : ℕ), j ≥ 1 → P j = 0:= by
        --  intro j hj
        --  exact h₂ j hj
        hole
      have h₅ : (∑' j : Set.Ici 1, (1 + x ^ (j : ℕ)) * P j) = 0:=
        by
        have h₆ : ∀ (j : Set.Ici 1), (1 + x ^ (j : ℕ)) * P j = 0:=
          by
          --  intro j
          have h₇ : P j = 0:= by
            have h₈ : (j : ℕ) ≥ 1:= by -- simpa using j.prop
              hole
            --  exact h₄ (j : ℕ) h₈
            hole
          --  --  rw [h₇] <;> simp
