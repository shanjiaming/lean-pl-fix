theorem putnam_2007_a3 (k : ℕ) (kpos : k > 0) (perms : Set (Fin (3 * k + 1) → ℤ)) (goodperms : Set (Fin (3 * k + 1) → ℤ))
  ...
  have h_perms_infinite : perms.Infinite:= by
    --  rw [hperms]
    have h₁ : ∀ (n : ℤ), (n : ℤ) ∈ Set.Icc 1 (3 * k + 1) → (n : ℤ) ≥ 1 ∧ (n : ℤ) ≤ 3 * k + 1:=
      by
      --  intro n hn
      --  simp only [Set.mem_Icc] at hn
      exact hn
      hole
    have h₂ : ∃ (f : Fin (3 * k + 1) → ℤ), (∀ y ∈ Icc 1 (3 * k + 1), ∃! x : Fin (3 * k + 1), f x = y):=
      by
      --  use fun i => (i : ℕ) + 1
      intro y hy
      ...
      hole
    --  rcases h₂ with ⟨f, hf⟩
    have h₃ : f ∈ {f : Fin (3 * k + 1) → ℤ | ∀ y ∈ Icc 1 (3 * k + 1), ∃! x : Fin (3 * k + 1), f x = y}:= by -- exact hf
      norm_cast
    ...
    hole
  have h_perms_ncard : perms.ncard = 0:=
    by
    have h₁ : perms.Infinite := h_perms_infinite
    have h₂ : perms.ncard = 0:= by
      have h₃ : perms.Infinite := h₁
      have h₄ : perms.ncard = 0:= by -- apply Set.Infinite.ncard_eq_zero h₃
        hole
      --  exact h₄
      linarith
    --  exact h₂
    linarith
  have h_goodperms_ncard : goodperms.ncard = 0:=
    by
    have h₁ : goodperms ⊆ perms:= by
      ...
      hole
    have h₂ : perms.Infinite := h_perms_infinite
    have h₃ : goodperms.Infinite ∨ goodperms.Finite:=
      by
