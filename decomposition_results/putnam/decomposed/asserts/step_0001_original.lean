theorem putnam_1974_a3 (assmption : ∀ (p : ℕ), Nat.Prime p ∧ p > 2 → ((∃ m n, (↑p : ℤ) = m ^ 2 + n ^ 2) ↔ p ≡ 1 [MOD 4])) : ∀ (p : ℕ),
    ((Nat.Prime p ∧ p > 2 ∧ ∃ x y, (↑p : ℤ) = x ^ 2 + 16 * y ^ 2) ↔
        p ∈ ({p | Nat.Prime p ∧ p ≡ 1 [MOD 8]}, {p | Nat.Prime p ∧ p ≡ 5 [MOD 8]}).1) ∧
      ((Nat.Prime p ∧ p > 2 ∧ ∃ x y, (↑p : ℤ) = 4 * x ^ 2 + 4 * x * y + 5 * y ^ 2) ↔
