...
: a * b ≤ n := by
  have h_main : a * b ≤ n := by
    ...
    have h1 : a * b ≤ n := by
      haveI : Fact (1 ≤ n) := ⟨npos⟩
      have h2 : (a : ℕ) ≤ n := by admit
      have h3 : (b : ℕ) ≤ n := by admit
      have h4 : 1 ≤ a := by admit
      have h5 : 1 ≤ b := by admit
      ...
      have h7 : ∀ i : Fin a, (∑ k : Fin n, H (rri i) k * H (rri i) k : ℝ) = n := by
        intro i
        have h8 : (∑ k : Fin n, H (rri i) k * H (rri i) k : ℝ) = ∑ k : Fin n, (H (rri i) k : ℝ) * (H (rri i) k : ℝ) := by admit
        rw [h8]
