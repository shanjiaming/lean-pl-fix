theorem putnam_2017_a1 (IsQualifying : Set ℤ → Prop)
  (IsQualifying_def :
    ∀ S, IsQualifying S ↔ (∀ n ∈ S, 0 < n) ∧ 2 ∈ S ∧ (∀ n > 0, n ^ 2 ∈ S → n ∈ S) ∧ (∀ n ∈ S, (n + 5) ^ 2 ∈ S))
  (S : Set ℤ) (hS : IsLeast IsQualifying S) : Sᶜ ∩ {n | 0 < n} = (({x : ℤ | x > 0 ∧ (x = 1 ∨ 5 ∣ x)}) : Set ℤ):=
  by
  have h₁ : 1 ∉ S:= by
    --  by_contra h
  ...
  have h₂ : ∀ (x : ℤ), x > 0 → 5 ∣ x → x ∉ S:= by
    ...
    have h₈ : 0 < x := h₃ x h₇
    have h₉ : 5 ∣ x := hx_div
    have h₁₀ : ∀ (n : ℤ), n ∈ S → 0 < n → 5 ∣ n → False:=
      by
      --  intro n hn_in_S hn_pos hn_div
      have h₁₁ : (n + 5 : ℤ) ^ 2 ∈ S := h₆ n hn_in_S
      have h₁₂ : 0 < (n + 5 : ℤ) ^ 2:= by -- nlinarith
        nlinarith
      have h₁₃ : 5 ∣ (n + 5 : ℤ) ^ 2:= by
        have h₁₄ : 5 ∣ n := hn_div
        have h₁₅ : 5 ∣ (n + 5 : ℤ):= by -- omega
          omega
        have h₁₆ : 5 ∣ (n + 5 : ℤ) ^ 2:= by -- exact dvd_pow h₁₅ (by norm_num)
          hole
        --  exact h₁₆
        omega
      have h₁₇ : (n + 5 : ℤ) ^ 2 ∈ S := h₁₁
      ...
      have h₂₀ : False:=
        ...
        have h₂₁ : IsQualifying (S \ {x | 5 ∣ x}):= by
          ...
          --  constructor
          --  · intro n hn
            have h₂₂ : n ∈ S:= by exact hn.1
              hole
            have h₂₃ : ¬5 ∣ n:= by exact hn.2
              hole
            have h₂₄ : 0 < n := h₃ n h₂₂
          --    exact h₂₄
          --  · constructor
                ...
                have h₃₄ : ¬5 ∣ (n + 5 : ℤ) ^ 2:= by
                  ...
                  have h₃₆ : 5 ∣ n + 5:= by
                    ...
                    have h₃₈ : 5 ∣ n + 5:= by
                      ...
                      have h₄₀ : 5 ∣ n + 5:= by
                        have h₄₁ : 5 ∣ (n + 5 : ℤ) ^ 2 := h₃₉
                        have h₄₂ : 5 ∣ n + 5:= by exact (Int.prime_iff_natAbs_prime.mpr (by norm_num)).dvd_of_dvd_pow h₄₁
                          hole
                        exact h₄₂
                        hole
                      exact h₄₀
    ...
    --  exact h₂₁
    hole
  have h₃ : Sᶜ ∩ {n | 0 < n} = {x : ℤ | x > 0 ∧ (x = 1 ∨ 5 ∣ x)}:=
    by
    --  apply Set.ext
    --  intro x
            ...
            have h₁₁ : x ∈ S:= by
              ...
              have h₁₇ : x ∈ S:= by
                ...
                have h₂₅ : IsQualifying (S ∪ { x }):= by
                  ...
                  --          exact Or.inl h₃₁
                        ·
                          have h₃₂ : n ^ 2 = x:= by -- simp_all [Set.mem_union, Set.mem_singleton_iff] <;> aesop
                            hole
                          have h₃₃ : n > 0 := hn_pos
                          have h₃₄ : n ^ 2 = x := h₃₂
                          have h₃₅ : n ∈ S:= by
