...
theorem putnam_1967_a3 :
  ...
  have h_is_least : ∀ a' ∈ {a | ∃ P : Polynomial ℤ, P.degree = 2 ∧ (∃ z1 z2 : Set.Ioo (0 : ℝ) 1, z1 ≠ z2 ∧ aeval (z1 : ℝ) P = 0 ∧ aeval (z2 : ℝ) P = 0) ∧ P.coeff 2 = a ∧ a > 0}, (5 : ℕ) ≤ a' := by
    ...
    have hP₅ : P.coeff 2 = (a' : ℤ) := by hole_4
    have hP₆ : a' > 0 := hP₃
    have h₁ : (a' : ℕ) ≥ 5 := by
      by_contra h
      ...
      have h₆ : P.coeff 2 = (a' : ℤ) := by hole_9
      have h₇ : a' > 0 := hP₃
      ...
      interval_cases a' <;> norm_num at hP₅ ⊢ <;>
        ...
        (try {
          ...
          (try {
            ...
            have h₁₇ : (P.map (algebraMap ℤ ℝ)).eval ↑z₁ = 0 := by hole_20
            have h₁₈ : (P.map (algebraMap ℤ ℝ)).eval ↑z₂ = 0 := by hole_21
