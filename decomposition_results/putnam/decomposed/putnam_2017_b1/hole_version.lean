...
theorem putnam_2017_b1
  ...
  have h_forward : L1 ∩ L2 ≠ ∅ → (∀ lambda : ℝ, lambda ≠ 0 → ∀ P, (P ∉ L1 ∧ P ∉ L2) → ∃ A1 A2, A1 ∈ L1 ∧ A2 ∈ L2 ∧ (A2 - P = lambda • (A1 - P))) := by
    ...
    have h5 : L1 ∈ {L | ∃ v w : Fin 2 → ℝ, w ≠ 0 ∧ L = {p | ∃ t : ℝ, p = v + t • w}} := by hole_2
    have h6 : L2 ∈ {L | ∃ v w : Fin 2 → ℝ, w ≠ 0 ∧ L = {p | ∃ t : ℝ, p = v + t • w}} := by hole_3
    ...
    have h7 : ∃ (p : Fin 2 → ℝ), p ∈ {p | ∃ t : ℝ, p = v1 + t • w1} ∩ {p | ∃ t : ℝ, p = v2 + t • w2} := by
      ...
      hole_4
    obtain ⟨p, hp⟩ := h7
    have h8 : p ∈ {p | ∃ t : ℝ, p = v1 + t • w1} := by hole_7
    have h9 : p ∈ {p | ∃ t : ℝ, p = v2 + t • w2} := by hole_8
    obtain ⟨t1, ht1⟩ := h8
    obtain ⟨t2, ht2⟩ := h9
    have h10 : p = v1 + t1 • w1 := by hole_9
    have h11 : p = v2 + t2 • w2 := by hole_10
    have h12 : v1 + t1 • w1 = v2 + t2 • w2 := by
      hole_11
    
    
    have h13 : ∃ A1 A2, A1 ∈ {p | ∃ t : ℝ, p = v1 + t • w1} ∧ A2 ∈ {p | ∃ t : ℝ, p = v2 + t • w2} ∧ (A2 - P = lambda • (A1 - P)) := by
      have h14 : P ∉ {p | ∃ t : ℝ, p = v1 + t • w1} := by hole_13
      have h15 : P ∉ {p | ∃ t : ℝ, p = v2 + t • w2} := by hole_14
        ...
        have h19 : ∃ (A1 : Fin 2 → ℝ), A1 ∈ {p | ∃ t : ℝ, p = v1 + t • w1} ∧ (P + lambda • (A1 - P)) ∈ {p | ∃ t : ℝ, p = v2 + t • w2} := by
          ...
          have h22 : ∃ (t : ℝ), P + lambda • ((v1 + t • w1) - P) = v2 + t • w2 := by
            ...
            have h23 : P + lambda • ((v1 + 0 • w1) - P) = v2 + 0 • w2 := by
              have h24 : P + lambda • ((v1 + 0 • w1) - P) = P + lambda • (v1 - P) := by hole_24
              have h25 : v2 + 0 • w2 = v2 := by hole_25
  ...
  have h_backward : (∀ lambda : ℝ, lambda ≠ 0 → ∀ P, (P ∉ L1 ∧ P ∉ L2) → ∃ A1 A2, A1 ∈ L1 ∧ A2 ∈ L2 ∧ (A2 - P = lambda • (A1 - P))) → L1 ∩ L2 ≠ ∅ := by
    ...
    have h₅ : L1 ∈ {L | ∃ v w : Fin 2 → ℝ, w ≠ 0 ∧ L = {p | ∃ t : ℝ, p = v + t • w}} := by hole_29
    have h₆ : L2 ∈ {L | ∃ v w : Fin 2 → ℝ, w ≠ 0 ∧ L = {p | ∃ t : ℝ, p = v + t • w}} := by hole_30
