theorem putnam_1987_b6 (p : ℕ) (F : Type*) [Field F] [Fintype F] (S : Set F) (hp : Odd p ∧ Nat.Prime p)
  (Fcard : Fintype.card F = p ^ 2) (Snz : ∀ x ∈ S, x ≠ 0) (Scard : S.ncard = ((p : ℤ) ^ 2 - 1) / 2)
  (hS : ∀ a : F, a ≠ 0 → Xor' (a ∈ S) (-a ∈ S)) : (Even ((S ∩ {x | ∃ a ∈ S, x = 2 * a}).ncard)):=
  by
  have h_char_ne_two : ringChar F ≠ 2:=
    by
    have h1 : (p : ℕ) ≠ 0:= by
      have h2 := hp.2.pos
      aesop
    ...
    have h2 : ringChar F = p:= by
      ...
      have h4 : ringChar F ∣ p:= by
        have h5 : ringChar F ∣ Fintype.card F:= by exact?
          hole
      ...
      have h11 : ringChar F = 1 ∨ ringChar F = p:=
        by
        have h12 : Nat.Prime p := hp.2
        exact?
        hole
      have h13 : ringChar F ≠ 1:=
        by
        have h14 : ringChar F = 1 → False:= by
          ...
          have h17 : (ringChar F : ℕ) = 1:= by simpa [h16] using ringChar.natCast (F := F)
            hole
          have h18 : ringChar F ∣ Fintype.card F:= by exact?
            hole
          have h19 : (ringChar F : ℕ) ∣ Fintype.card F:= by exact_mod_cast h18
            hole
