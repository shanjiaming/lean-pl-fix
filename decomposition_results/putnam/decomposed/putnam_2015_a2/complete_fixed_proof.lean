theorem putnam_2015_a2 (a : ℕ → ℤ) (abase : a 0 = 1 ∧ a 1 = 2) (arec : ∀ n ≥ 2, a n = 4 * a (n - 1) - a (n - 2)) :
  Odd ((181) : ℕ) ∧ ((181) : ℕ).Prime ∧ ((((181) : ℕ) : ℤ) ∣ a 2015):=
  by
  have h_odd : Odd (181 : ℕ):= by -- -- norm_num [Nat.odd_iff_not_even, Nat.even_iff, Nat.mod_eq_of_lt] <;> decide
    norm_cast
  have h_prime : Nat.Prime (181 : ℕ):= by -- norm_num [Nat.Prime] <;> decide
    norm_num
  have h_div : ((181 : ℕ) : ℤ) ∣ a 2015:=
    by
    have h₁ : ((181 : ℕ) : ℤ) ∣ a 2015:=
      by
      have h₂ : a 2015 % (181 : ℤ) = 0:=
        by
        have h₃ : ∀ n : ℕ, a n % (181 : ℤ) = a (n % 20) % (181 : ℤ):=
          by
          --  intro n
          have h₄ : ∀ n : ℕ, a n % (181 : ℤ) = a (n % 20) % (181 : ℤ):=
            by
