theorem putnam_2021_a2 (g : ℝ → ℝ)
  (hg : ∀ x > 0, Tendsto (fun r : ℝ => ((x + 1) ^ (r + 1) - x ^ (r + 1)) ^ (1 / r)) (𝓝[>] 0) (𝓝 (g x))) :
  Tendsto (fun x : ℝ => g x / x) atTop (𝓝 ((Real.exp 1) : ℝ)):=
  by
  have h_main : Tendsto (fun x => g x / x) atTop (𝓝 (Real.exp 1)):=
    by
    have h₁ : Tendsto (fun x : ℝ => (1 + 1 / x : ℝ) ^ (x + 1)) atTop (𝓝 (Real.exp 1)):=
      by
      have h₂ : Tendsto (fun x : ℝ => (1 + 1 / x : ℝ) ^ x) atTop (𝓝 (Real.exp 1)):=
        by
      ...
      have h₄ : Tendsto (fun x : ℝ => (1 + 1 / x : ℝ) ^ (x + 1)) atTop (𝓝 (Real.exp 1)):=
        ...
        have h₅ : Tendsto (fun x : ℝ => (1 + 1 / x : ℝ) ^ (x + 1)) atTop (𝓝 (Real.exp 1)):=
          by
          have h₆ : (fun x : ℝ => (1 + 1 / x : ℝ) ^ (x + 1)) = (fun x : ℝ => (1 + 1 / x : ℝ) ^ x * (1 + 1 / x : ℝ)):=
            by
            --  funext x
      ...
      --  exact h₄
      hole
    have h₂ : ∀ (x : ℝ), x > 0 → g x / x = (1 + 1 / x : ℝ) ^ (x + 1):=
      by
      --  intro x hx
      ...
      have h₄ : g x = (x + 1) ^ (x + 1) / x ^ x:=
        ...
        have h₆ : g x = (x + 1) ^ (x + 1) / x ^ x:=
          ...
          have h₁₀ : g x = (x + 1) ^ (x + 1) / x ^ x:=
            ...
            --  exact h₁₃
            hole
          have h₇ : Tendsto (fun r : ℝ => ((x + 1) ^ (r + 1) - x ^ (r + 1)) ^ (1 / r)) (𝓝[>] 0) (𝓝 ((x + 1) ^ (x + 1) / x ^ x)):=
            by
            have h₈ :
              Tendsto (fun r : ℝ => ((x + 1) ^ (r + 1) - x ^ (r + 1)) ^ (1 / r)) (𝓝[>] 0) (𝓝 ((x + 1) ^ (x + 1) / x ^ x)) :=
              by
        ...
        --  exact h₆
        hole
      have h₅ : g x / x = (1 + 1 / x : ℝ) ^ (x + 1):=
        by
        have h₆ : g x = (x + 1) ^ (x + 1) / x ^ x := h₄
        --  rw [h₆]
        have h₇ : (x + 1 : ℝ) ^ (x + 1) / x ^ x / x = (1 + 1 / x : ℝ) ^ (x + 1):=
          by
          ...
          have h₁₀ : (x + 1 : ℝ) ^ (x + 1) / x ^ x / x = (1 + 1 / x : ℝ) ^ (x + 1):=
            ...
            have h₁₂ : (x + 1 : ℝ) ^ (x + 1) / x ^ x = (x + 1 : ℝ) ^ (x + 1) / x ^ x:= by -- rfl
              norm_num
            --  rw [h₁₂]
            have h₁₃ : ((x + 1 : ℝ) ^ (x + 1) / x ^ x) / x = (x + 1 : ℝ) ^ (x + 1) / (x ^ x * x):= by -- field_simp <;> ring_nf
              ring
            --  rw [h₁₃]
            have h₁₄ : (x + 1 : ℝ) ^ (x + 1) / (x ^ x * x) = (x + 1 : ℝ) ^ (x + 1) / x ^ (x + 1):=
              by
              have h₁₅ : (x : ℝ) ^ x * x = (x : ℝ) ^ (x + 1):=
                by
              ...
              hole
            --  rw [h₁₄]
            have h₁₅ : (x + 1 : ℝ) ^ (x + 1) / x ^ (x + 1) = (1 + 1 / x : ℝ) ^ (x + 1):=
              by
              have h₁₆ : (x + 1 : ℝ) ^ (x + 1) / x ^ (x + 1) = ((x + 1 : ℝ) / x) ^ (x + 1):=
                by
                have h₁₇ : (x + 1 : ℝ) ^ (x + 1) / x ^ (x + 1) = ((x + 1 : ℝ) / x) ^ (x + 1):= by
                  --  rw [div_rpow (by positivity) (by positivity)] <;> field_simp <;> ring_nf <;> field_simp
                  hole
                --  rw [h₁₇]
                linarith
              --  rw [h₁₆]
              have h₁₇ : ((x + 1 : ℝ) / x : ℝ) = (1 + 1 / x : ℝ):= by -- field_simp <;> ring_nf <;> field_simp <;> linarith
                field_simp
              --  rw [h₁₇]
              hole
            --  rw [h₁₅]
      ...
      --  exact h₅
      hole
    have h₃ : Tendsto (fun x : ℝ => g x / x) atTop (𝓝 (Real.exp 1)):=
      by
