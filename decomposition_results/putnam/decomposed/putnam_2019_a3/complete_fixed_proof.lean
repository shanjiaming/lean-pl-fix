theorem putnam_2019_a3 (v : Polynomial ℂ → Prop)
  (hv :
    v = fun b =>
      b.degree = 2019 ∧
        1 ≤ (b.coeff 0).re ∧
  ...
  (μ : Polynomial ℂ → ℝ) (hμ : μ = fun b => (Multiset.map (fun ω : ℂ => ‖ω‖) (Polynomial.roots b)).sum / 2019) :
  ...
  have h_no_poly : ¬∃ (b : Polynomial ℂ), v b:= by
    ...
    have h₆ : (b.coeff 2019).re > (b.coeff 0).re:=
      by
      have h₆₁ : (b.coeff 0).re < (b.coeff 1).re:=
        by
        have h₆₂ := h₅ ⟨0, by norm_num⟩
        --  simpa using h₆₂
        linarith
      have h₆₂ : ∀ (n : ℕ), n ≤ 2019 → (b.coeff 0).re + n ≤ (b.coeff n).re:=
        by
        --  intro n hn
    ...
    --  linarith
    hole
  have h_main : IsGreatest {M : ℝ | ∀ b, v b → μ b ≥ M} ((2019 ^ (-(1 : ℝ) / 2019)) : ℝ):=
    by
    have h₁ : ∀ (b : Polynomial ℂ), v b → μ b ≥ (2019 : ℝ) ^ (-(1 : ℝ) / 2019):=
      by
      --  intro b hb
      --  exfalso
      --  exact h_no_poly ⟨b, hb⟩
      hole
    have h₂ : (2019 : ℝ) ^ (-(1 : ℝ) / 2019) ∈ {M : ℝ | ∀ b, v b → μ b ≥ M}:= by -- simpa using h₁
      hole
    have h₃ : ∀ (y : ℝ), y ∈ {M : ℝ | ∀ b, v b → μ b ≥ M} → y ≤ (2019 : ℝ) ^ (-(1 : ℝ) / 2019):=
      by
      --  intro y hy
