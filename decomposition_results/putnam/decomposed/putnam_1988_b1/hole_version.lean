...
theorem putnam_1988_b1
  ...
  have h_main : ∃ (x y z : ℤ), x > 0 ∧ y > 0 ∧ z > 0 ∧ a * b = x * y + x * z + y * z + 1 := by
    use 1, (a - 1 : ℤ), (b - 1 : ℤ)
    have h₁ : (1 : ℤ) > 0 := by hole_3
    have h₂ : (a - 1 : ℤ) > 0 := by
      have h₂₁ : (a : ℤ) ≥ 2 := by hole_5
      have h₂₂ : (a - 1 : ℤ) > 0 := by hole_6
      hole_4
    have h₃ : (b - 1 : ℤ) > 0 := by
      have h₃₁ : (b : ℤ) ≥ 2 := by hole_8
      have h₃₂ : (b - 1 : ℤ) > 0 := by hole_9
      hole_7
    have h₄ : (a * b : ℤ) = (1 : ℤ) * (a - 1 : ℤ) + (1 : ℤ) * (b - 1 : ℤ) + (a - 1 : ℤ) * (b - 1 : ℤ) + 1 := by
      have h₄₁ : (a : ℤ) ≥ 2 := by hole_11
      have h₄₂ : (b : ℤ) ≥ 2 := by hole_12
      hole_10
    hole_2
