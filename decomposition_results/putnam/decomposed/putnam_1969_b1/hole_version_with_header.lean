...
: 24 ∣ ∑ d in divisors n, d := by
  have h₀ : n % 2 = 1 := by
    have h₁ : (n + 1) % 2 = 0 := by hole_1
    hole_2
  
  have h₁ : n % 8 = 7 := by
    have h₂ : (n + 1) % 24 = 0 := by hole_3
    hole_4
  ...
  have h₃ : ¬ IsSquare n := by
    ...
    have h₄ : k ^ 2 % 8 = 0 ∨ k ^ 2 % 8 = 1 ∨ k ^ 2 % 8 = 4 := by
      have : k % 8 = 0 ∨ k % 8 = 1 ∨ k % 8 = 2 ∨ k % 8 = 3 ∨ k % 8 = 4 ∨ k % 8 = 5 ∨ k % 8 = 6 ∨ k % 8 = 7 := by hole_7
      hole_8
    have h₅ : k ^ 2 % 8 = 0 ∨ k ^ 2 % 8 = 1 ∨ k ^ 2 % 8 = 4 := by hole_9
    have h₆ : (k ^ 2) % 8 = 7 → False := by
      hole_10
    have h₇ : (k ^ 2) % 8 = 7 → False := by hole_11
    have h₈ : (k ^ 2) % 8 ≠ 7 := by
      hole_12
    hole_13
  ...
  have h₄ : ∀ (d : ℕ), d ∣ n → (d + n / d) % 8 = 0 := by
    ...
    have h₉ : (d % 8 = 1 ∧ (n / d) % 8 = 7) ∨ (d % 8 = 7 ∧ (n / d) % 8 = 1) ∨ (d % 8 = 3 ∧ (n / d) % 8 = 5) ∨ (d % 8 = 5 ∧ (n / d) % 8 = 3) := by
      have h₁₀ : d % 8 = 1 ∨ d % 8 = 3 ∨ d % 8 = 5 ∨ d % 8 = 7 := by hole_21
