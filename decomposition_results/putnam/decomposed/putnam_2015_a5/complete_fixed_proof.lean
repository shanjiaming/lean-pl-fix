theorem putnam_2015_a5 (q : ℕ) (Nq : ℕ) (qodd : Odd q) (qpos : q > 0)
  (hNq : Nq = {a : ℕ | 0 < a ∧ a < (q : ℝ) / 4 ∧ Nat.gcd a q = 1}.encard) :
  Odd Nq ↔ ∃ p k : ℕ, q = p ^ k ∧ k > 0 ∧ p.Prime ∧ (p % 8 = 5 ∨ p % 8 = 7):=
  by
  have h_q_eq_one : q = 1 → (Odd Nq ↔ ∃ p k : ℕ, q = p ^ k ∧ k > 0 ∧ p.Prime ∧ (p % 8 = 5 ∨ p % 8 = 7)):=
    by
    --  intro hq
    have h₁ : q = 1 := hq
    have h₂ : Nq = 0:= by
    ...
    have h₃ : ¬Odd Nq:= by
      ...
      --  omega
      hole
    have h₄ : ¬∃ p k : ℕ, q = p ^ k ∧ k > 0 ∧ p.Prime ∧ (p % 8 = 5 ∨ p % 8 = 7):=
      by
      --  intro h
      ...
      have h₆ : p = 1:= by
        ...
        have h₈ : p = 1:= by
          ...
          have h₁₁ : p ^ k ≥ p:= by
            have h₁₂ : k ≥ 1:= by -- omega
              hole
            have h₁₃ : p ^ k ≥ p ^ 1:= by -- exact Nat.pow_le_pow_of_le_right (by omega) h₁₂
              hole
            --  simpa using h₁₃
            hole
          have h₁₂ : p ^ k > 1:= by
        ...
        --  exact h₈
        hole
      have h₇ : ¬p.Prime:= by -- simp_all [Nat.Prime]
        hole
      --  contradiction
      hole
    --  --  --  --  constructor <;> intro h <;> simp_all [h₃, h₄] <;> try tauto
    hole
  have h_q_gt_one : q > 1 → (Odd Nq ↔ ∃ p k : ℕ, q = p ^ k ∧ k > 0 ∧ p.Prime ∧ (p % 8 = 5 ∨ p % 8 = 7)):=
    by
    --  intro hq_gt_one
    have h₁ : Odd Nq ↔ ∃ p k : ℕ, q = p ^ k ∧ k > 0 ∧ p.Prime ∧ (p % 8 = 5 ∨ p % 8 = 7):=
      by
    ...
    --  exact h₁
    hole
  have h_main : Odd Nq ↔ ∃ p k : ℕ, q = p ^ k ∧ k > 0 ∧ p.Prime ∧ (p % 8 = 5 ∨ p % 8 = 7):=
    by
    --  by_cases h : q = 1
    --  · exact h_q_eq_one h
    ·
