...
theorem putnam_2016_b5
  ...
  · 
    ...
    have h_forward : ∀ x y : Set.Ioi (1 : ℝ), ((x : ℝ) ^ 2 ≤ y ∧ y ≤ (x : ℝ) ^ 3) → ((f x : ℝ) ^ 2 ≤ f y ∧ f y ≤ (f x : ℝ) ^ 3) := by
      ...
      intro x y hxy
      have h₁ : (f x : ℝ) = (x : ℝ) ^ c := by hole_3
      have h₂ : (f y : ℝ) = (y : ℝ) ^ c := by hole_4
      have h₃ : (x : ℝ) ^ 2 ≤ (y : ℝ) := hxy.1
      have h₄ : (y : ℝ) ≤ (x : ℝ) ^ 3 := hxy.2
      have h₅ : (1 : ℝ) < (x : ℝ) := by hole_5
      have h₆ : (1 : ℝ) < (y : ℝ) := by
        have h₇ : (1 : ℝ) < (x : ℝ) := x.2
        have h₈ : (x : ℝ) ^ 2 ≤ (y : ℝ) := hxy.1
        have h₉ : (1 : ℝ) < (x : ℝ) ^ 2 := by
      ...
      have h₇ : (f x : ℝ) ^ 2 ≤ (f y : ℝ) := by
        rw [h₁, h₂]
        have h₈ : (0 : ℝ) < c := by hole_9
        have h₉ : (0 : ℝ) < (x : ℝ) := by hole_10
        have h₁₀ : (0 : ℝ) < (y : ℝ) := by hole_11
        have h₁₁ : (0 : ℝ) < (x : ℝ) ^ c := by hole_12
        have h₁₂ : (0 : ℝ) < (y : ℝ) ^ c := by hole_13
        
        have h₁₃ : (x : ℝ) ^ 2 ≤ (y : ℝ) := hxy.1
        have h₁₄ : (0 : ℝ) < (x : ℝ) ^ 2 := by hole_14
        have h₁₅ : (0 : ℝ) < (y : ℝ) := by hole_15
      ...
      have h₈ : (f y : ℝ) ≤ (f x : ℝ) ^ 3 := by
        rw [h₁, h₂]
        have h₉ : (0 : ℝ) < c := by hole_22
        have h₁₀ : (0 : ℝ) < (x : ℝ) := by hole_23
        have h₁₁ : (0 : ℝ) < (y : ℝ) := by hole_24
        have h₁₂ : (0 : ℝ) < (x : ℝ) ^ c := by hole_25
        have h₁₃ : (0 : ℝ) < (y : ℝ) ^ c := by hole_26
        
        have h₁₄ : (y : ℝ) ≤ (x : ℝ) ^ 3 := hxy.2
        have h₁₅ : (0 : ℝ) < (x : ℝ) ^ 3 := by hole_27
        have h₁₆ : (0 : ℝ) < (y : ℝ) := by hole_28
