      ...
      · 
        ...
        have h₃ : ⌊(n : ℝ) / k⌋ = 0 := by
          have h₃₁ : (n : ℝ) / k < 1 := by
            have h₃₂ : (n : ℤ) < k := by
              by_contra h₃₃
              have h₃₄ : (n : ℤ) ≥ k := by hole_8
              have h₃₅ : (n : ℝ) ≥ k := by hole_9
              have h₃₆ : (n : ℝ) / k ≥ 1 := by
                have h₃₇ : (n : ℝ) ≥ k := by hole_11
              ...
              have h₃₁₅ : ⌊(n : ℝ) / k⌋ ^ k = 0 := by
                hole_19
              hole_7
            have h₃₁₆ : (n : ℝ) < k := by hole_20
            have h₃₁₇ : (n : ℝ) / k < 1 := by
              hole_21
            hole_6
          have h₃₁₈ : ⌊(n : ℝ) / k⌋ ≤ (n : ℝ) / k := by hole_22
          have h₃₁₉ : ⌊(n : ℝ) / k⌋ < 1 := by
            hole_23
          have h₃₂₀ : ⌊(n : ℝ) / k⌋ ≤ 0 := by
            by_contra h₃₂₁
            have h₃₂₂ : ⌊(n : ℝ) / k⌋ ≥ 1 := by
              have h₃₂₃ : ⌊(n : ℝ) / k⌋ > 0 := by hole_26
              have h₃₂₄ : ⌊(n : ℝ) / k⌋ ≥ 1 := by
                hole_27
              hole_25
            have h₃₂₅ : (n : ℝ) / k ≥ 1 := by
              have h₃₂₆ : (⌊(n : ℝ) / k⌋ : ℝ) ≥ 1 := by hole_29
              hole_28
            hole_24
          have h₃₂₁ : ⌊(n : ℝ) / k⌋ = 0 := by
            have h₃₂₂ : ⌊(n : ℝ) / k⌋ ≤ 0 := by hole_31
            have h₃₂₃ : ⌊(n : ℝ) / k⌋ ≥ 0 := by
              hole_32
            have h₃₂₄ : ⌊(n : ℝ) / k⌋ = 0 := by
              hole_33
