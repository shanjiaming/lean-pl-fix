...
: f.eval n ∣ f.eval (f.eval n + 1) ↔ n = 1 := by
  have h1 : Polynomial.eval 1 f > 0 := by
    ...
    have h12 : Polynomial.eval 1 f ≥ (f.coeff m : ℤ) := by
      hole_11
    have h13 : Polynomial.eval 1 f > 0 := by hole_12
    hole_2
  ...
  have h3 : n ≠ 1 → Polynomial.eval n f > Polynomial.eval 1 f := by
    ...
    have h4 : n ≥ 2 := by
      by_contra h
      have h5 : n ≤ 1 := by hole_16
      have h6 : n = 1 := by hole_17
      hole_15
    have h5 : (n : ℤ) ≥ 2 := by hole_18
    ...
    have h6 : Polynomial.eval n f > Polynomial.eval 1 f := by
      ...
      have h9 : (Polynomial.eval n f : ℤ) > (Polynomial.eval 1 f : ℤ) := by
        ...
        have h10 : (Polynomial.eval n f : ℤ) = Polynomial.eval n f := by hole_22
        have h11 : (Polynomial.eval 1 f : ℤ) = Polynomial.eval 1 f := by hole_23
        have h12 : (Polynomial.eval n f : ℤ) - (Polynomial.eval 1 f : ℤ) > 0 := by
          
          have h13 : (Polynomial.eval n f : ℤ) - (Polynomial.eval 1 f : ℤ) = Polynomial.eval n f - Polynomial.eval 1 f := by hole_25
          rw [h13]
          have h14 : Polynomial.eval n f - Polynomial.eval 1 f > 0 := by
            
            have h15 : 0 < (n : ℤ) := by hole_27
            ...
            have h18 : ∑ k in f.support, (f.coeff k : ℤ) * (n ^ k - 1 : ℤ) > 0 := by
              ...
              have h20 : (f.coeff m : ℤ) * (n ^ m - 1 : ℤ) > 0 := by
                ...
                have h22 : (n : ℤ) ^ m - 1 ≥ 1 := by
                  have h23 : (n : ℤ) ≥ 2 := by hole_38
                  have h24 : (n : ℤ) ^ m ≥ (n : ℤ) ^ 1 := by
                    hole_39
                  have h25 : (n : ℤ) ^ 1 = (n : ℤ) := by hole_40
  ...
  have h4 : n = 1 → Polynomial.eval n f ∣ Polynomial.eval (Polynomial.eval n f + 1) f := by
    ...
    have h5 : Polynomial.eval 1 f ∣ Polynomial.eval (Polynomial.eval 1 f + 1) f := by
      have h6 : Polynomial.eval (Polynomial.eval 1 f + 1) f = Polynomial.eval (1 + Polynomial.eval 1 f) f := by hole_54
      rw [h6]
  ...
  have h6 : f.eval n ∣ f.eval (f.eval n + 1) ↔ n = 1 := by
    constructor
    · intro h
      have h₁ : Polynomial.eval n f ∣ Polynomial.eval (Polynomial.eval n f + 1) f := by hole_71
      have h₂ : n = 1 := h5 h₁
      exact h₂
    · intro h
      have h₁ : n = 1 := h
