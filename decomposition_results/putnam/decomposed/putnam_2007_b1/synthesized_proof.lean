...
: f.eval n ∣ f.eval (f.eval n + 1) ↔ n = 1 := by
  ...
  have h3 : n ≠ 1 → Polynomial.eval n f > Polynomial.eval 1 f := by
    ...
    have h4 : n ≥ 2 := by
      ...
      have h6 : n = 1 := by linarith
      omega
    have h5 : (n : ℤ) ≥ 2 := by linarith
    ...
    have h6 : Polynomial.eval n f > Polynomial.eval 1 f := by
      ...
      have h9 : (Polynomial.eval n f : ℤ) > (Polynomial.eval 1 f : ℤ) := by
        ...
        have h10 : (Polynomial.eval n f : ℤ) = Polynomial.eval n f := by norm_num
        have h11 : (Polynomial.eval 1 f : ℤ) = Polynomial.eval 1 f := by norm_num
        have h12 : (Polynomial.eval n f : ℤ) - (Polynomial.eval 1 f : ℤ) > 0 := by
          
          have h13 : (Polynomial.eval n f : ℤ) - (Polynomial.eval 1 f : ℤ) = Polynomial.eval n f - Polynomial.eval 1 f := by norm_num
          rw [h13]
            ...
            have h18 : ∑ k in f.support, (f.coeff k : ℤ) * (n ^ k - 1 : ℤ) > 0 := by
              ...
              have h20 : (f.coeff m : ℤ) * (n ^ m - 1 : ℤ) > 0 := by
                ...
                have h22 : (n : ℤ) ^ m - 1 ≥ 1 := by
                  have h23 : (n : ℤ) ≥ 2 := by linarith
                  have h24 : (n : ℤ) ^ m ≥ (n : ℤ) ^ 1 := by
                    admit
                  have h25 : (n : ℤ) ^ 1 = (n : ℤ) := by norm_num
  ...
  have h6 : f.eval n ∣ f.eval (f.eval n + 1) ↔ n = 1 := by
    constructor
    · intro h
      have h₁ : Polynomial.eval n f ∣ Polynomial.eval (Polynomial.eval n f + 1) f := by simpa
      have h₂ : n = 1 := h5 h₁
      exact h₂
    · intro h
      have h₁ : n = 1 := h
