theorem putnam_1973_b2 (z : ℂ) (hzrat : ∃ q1 q2 : ℚ, z.re = q1 ∧ z.im = q2) (hznorm : ‖z‖ = 1) :
  ∀ n : ℤ, ∃ q : ℚ, ‖z ^ (2 * n) - 1‖ = q:=
  by
  have h_main : ∀ n : ℤ, ∃ q : ℚ, ‖z ^ (2 * n) - 1‖ = q:=
    by
    --  intro n
    have h₁ : ‖z ^ (2 * n) - 1‖ = ‖z ^ n - (star z) ^ n‖:=
      by
      have h₂ : z ^ (2 * n) - 1 = z ^ n * (z ^ n - (star z) ^ n):=
        by
        have h₃ : z ^ (2 * n) = (z ^ n) ^ 2:= by
          ...
          --  simp [zpow_mul, zpow_ofNat] <;> ring_nf <;> simp [Complex.ext_iff, pow_two, mul_comm, mul_assoc, mul_left_comm] <;>
            ...
            norm_num
          hole
        have h₄ : (star z) ^ n = star (z ^ n):= by -- simp [star_pow]
          norm_num
        --  calc
    ...
    have h₂ : ∃ q : ℚ, ‖z ^ n - (star z) ^ n‖ = q:=
      ...
      have h₈ : (z.im : ℝ) = q2:= by -- exact_mod_cast hq2
        linarith
      have h₉ : ‖z‖ ^ 2 = 1:= by -- -- rw [hznorm] <;> norm_num
        nlinarith
      have h₁₀ : (z.re : ℝ) ^ 2 + (z.im : ℝ) ^ 2 = 1:=
        by
