      ...
      have h₃ : Tendsto (fun x : ℝ => (y x)^3) atTop (𝓝 0) := by
        ...
        have h₄ : ∀ ε : ℝ, 0 < ε → (∀ᶠ x in atTop, |(y x)^3| < ε) := by
          ...
          have h₅₅ : ∀ᶠ x in atTop, |(y x)^3| < ε := by
            ...
            have h₅₁₀ : -(ε : ℝ) < (y x)^3 := by
              by_contra h
              have h₅₁₁ : (y x)^3 ≤ -ε := by hole_14
              have h₅₁₂ : (deriv y x)^2 < 0 := by
                hole_15
              have h₅₁₃ : (deriv y x)^2 ≥ 0 := by hole_16
              hole_13
            have h₅₁₄ : |(y x)^3| < ε := by
              hole_17
            hole_9
    ...
    have h₂ : Tendsto y atTop (𝓝 0) := by
      ...
      have h₄ : Tendsto y atTop (𝓝 0) := by
        ...
        have h₆ : Tendsto (fun x : ℝ => y x) atTop (𝓝 0) := by
          ...
          have h₈ : Continuous (fun x : ℝ => x ^ (3 : ℕ) : ℝ → ℝ) := by hole_22
          have h₉ : Tendsto (fun x : ℝ => (y x)^3) atTop (𝓝 0) := h₅
          have h₁₀ : Tendsto (fun x : ℝ => y x) atTop (𝓝 0) := by
      ...
      have h₃ : Tendsto (fun x : ℝ => (y x)^3) atTop (𝓝 0) := by
        ...
        have h₄ : ∀ ε : ℝ, 0 < ε → (∀ᶠ x in atTop, |(y x)^3| < ε) := by
          ...
          have h₅₅ : ∀ᶠ x in atTop, |(y x)^3| < ε := by
            ...
            have h₅₁₀ : -(ε : ℝ) < (y x)^3 := by
              by_contra h
              have h₅₁₁ : (y x)^3 ≤ -ε := by hole_40
              have h₅₁₂ : (deriv y x)^2 < 0 := by
                hole_41
              have h₅₁₃ : (deriv y x)^2 ≥ 0 := by hole_42
              hole_39
            have h₅₁₄ : |(y x)^3| < ε := by
              hole_43
            hole_35
