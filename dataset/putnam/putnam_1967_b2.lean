...
: max (max A B) C ≥ 4 / 9 ∧ max (max α β) γ ≥ 4 / 9 := by
  ...
  have hB : B = 2 * p * (1 - p) := by
    ...
    <;> nlinarith [prbound.1, prbound.2.1, prbound.2.2.1, prbound.2.2.2, hA]
  ...
  have hC : C = (1 - p) ^ 2 := by
    ...
    have h₉ := id1 (-1) 2
    norm_num at h₁ h₂ h₃ h₄ h₅ h₆ h₇ h₈ h₉ ⊢
    <;> nlinarith [prbound.1, prbound.2.1, prbound.2.2.1, prbound.2.2.2, hA, hB]
    ...
    have h₃ : max (max A B) C ≥ 4 / 9 := by
      ...
      · -- Case 1: p ≤ 1/3
        ...
        have h₅' : max (max A B) C ≥ C := by
          apply le_trans _ (le_max_right (max A B) C)
          <;> simp [h₅]
        linarith
      · -- Case 2: p > 1/3
        by_cases h₅ : p ≥ 2 / 3
        · -- Subcase 2.1: p ≥ 2/3
          ...
          have h₆' : max (max A B) C ≥ A := by
            apply le_trans _ (le_max_left (max A B) C)
            <;> simp [h₆]
            <;> apply le_max_left
          linarith
        · -- Subcase 2.2: 1/3 < p < 2/3
          have h₆ : B ≥ 4 / 9 := by
            have h₆₁ : B = 2 * p * (1 - p) := hB
            rw [h₆₁]
