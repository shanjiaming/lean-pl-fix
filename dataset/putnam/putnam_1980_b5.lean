theorem putnam_1980_b5
  ...
  have h_main : (t ≤ 1) ↔ (∀ f ∈ S t, ∀ g ∈ S t, f * g ∈ S t) := by
    constructor
    · -- Prove the forward direction: if t ≤ 1, then S t is closed under multiplication
      intro h
      have h₁ : t ≤ 1 := h
      intro f hf g hg
      have h₂ : S t = {f : ℝ → ℝ | (∀ x ∈ T, f x ≥ 0) ∧ StrictMonoOn f T ∧ IsConvex f ∧ ContinuousOn f T ∧ P t f} := by
        ...
        <;> aesop
      rw [h₂] at hf hg ⊢
      have h₃ : (∀ x ∈ T, f x ≥ 0) ∧ StrictMonoOn f T ∧ IsConvex f ∧ ContinuousOn f T ∧ P t f := by simpa using hf
      have h₄ : (∀ x ∈ T, g x ≥ 0) ∧ StrictMonoOn g T ∧ IsConvex g ∧ ContinuousOn g T ∧ P t g := by simpa using hg
      have h₅ : (∀ x ∈ T, (f * g) x ≥ 0) := by
        intro x hx
        have h₅₁ : f x ≥ 0 := h₃.1 x hx
        have h₅₂ : g x ≥ 0 := h₄.1 x hx
        have h₅₃ : (f * g) x = f x * g x := by simp [Pi.mul_apply]
        rw [h₅₃]
        nlinarith
      have h₆ : StrictMonoOn (f * g) T := by
        ...
        have h₇₀ : (f * g) x = f x * g x := by simp [Pi.mul_apply]
        have h₇₁ : (f * g) y = f y * g y := by simp [Pi.mul_apply]
        rw [h₇₀, h₇₁]
        have h₇₂ : f x ≥ 0 := h₆₃ x h₆₅
        have h₇₃ : f y ≥ 0 := h₆₃ y h₆₆
        have h₇₄ : g x ≥ 0 := h₆₄ x h₆₅
      ...
      have h₇ : IsConvex (f * g) := by
        ...
        have h₇₃ : ∀ u ∈ T, ∀ v ∈ T, ∀ s ∈ T, f (s * u + (1 - s) * v) ≤ s * f u + (1 - s) * f v := by
          ...
          have h₇₄ : f (s * u + (1 - s) * v) ≤ s * f u + (1 - s) * f v := by
            have h₇₅ : IsConvex f := h₃.2.2.1
            rw [IsConvex_def] at h₇₅
            exact h₇₅ f u hu v hv s hs
          exact h₇₄
        ...
        have h₇₅ : IsConvex (f * g) := by
          ...
          have h₇₈ : (f * g) (s * u + (1 - s) * v) = f (s * u + (1 - s) * v) * g (s * u + (1 - s) * v) := by simp [Pi.mul_apply]
      ...
      have h₉ : P t (f * g) := by
        ...
        exact h₉₆
      exact ⟨h₅, h₆, h₇, h₈, h₉⟩
    · -- Prove the backward direction: if S t is closed under multiplication, then t ≤ 1
      intro h
      by_contra h₁
      ...
      have h₅ : (fun x : ℝ => x) ∈ S t := by
        rw [h₃]
        constructor
        · -- Prove that the identity function is nonnegative on T
          intro x hx
          have h₅₁ : x ∈ T := hx
          ...
          <;> linarith
        · constructor
          · -- Prove that the identity function is strictly increasing on T
            intro x hx y hy hxy
            have h₅₅ : x ∈ T := hx
            have h₅₆ : y ∈ T := hy
            have h₅₇ : x < y := hxy
            ...
            <;> linarith
          · constructor
            · -- Prove that the identity function is convex
              ...
              exact h₅₅
            · constructor
              · -- Prove that the identity function is continuous on T
                have h₅₅ : ContinuousOn (fun x : ℝ => x : ℝ → ℝ) T := by
                  apply continuousOn_id
                exact h₅₅
              · -- Prove that the identity function satisfies P t
